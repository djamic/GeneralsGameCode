...
void GameLogic::init(void) {
  ...
  setFPMode();
  // m_randomValue = new LogicRandomValue;
  ...
  for (i = 0; i < MAX_SLOTS; ++i) {
    ...
    if (posIdx < 0 || posIdx >= numPlayers) {
      ...
      if (hasStartSpotBeenPicked) {
        ...
        DEBUG_LOG(
            ("Setting start position %d to %d (random choice)", i, posIdx));
...
void GameLogic::startNewGame(Bool saveGame) {
  g_enableDjLog = true; // Enable logging at game start
  DjLog("GameLogic::startNewGame called. Logging enabled.");
...
#define DO_CIVILIAN
  ...
  if (timeFrames > 0) {
    ...
    if (mode == NO_SPAWN) {
      ...
      drawCallNoSpawn = (float)drawCallTotal /
                        (float)(TIME_FRAMES * 100); // 100 units for TIME_FRAMES
    }
