...
WindowTranslator::translateGameMessage(const GameMessage *msg) {
  ...
  if (TheTacticalView && TheTacticalView->isMouseLocked()) {
    ...
    // to in scrolling mode (via keyboard or mouse) and left click in the
    // controlbar. Without this code, the left click goes through the interface
    // ignoring buttons and blockage and ends up issuing orders right through
    // the controlbar!
  ...
  case GameMessage::MSG_META_TOGGLE_ATTACKMOVE: {
    ...
    // Force it to keep the message, regardless of what the window thinks it did
    // with the input.
    return KEEP_MESSAGE;
  }
  ...
  case GameMessage::MSG_RAW_KEY_UP: {
    ...
    if (returnCode != WIN_INPUT_USED && (key == KEY_ESC) &&
        (BitIsSet(state, KEY_STATE_UP)) && TheDisplay->isMoviePlaying() &&
    ...
    // INSERT key: Toggle AI Assist for local human player
    if (returnCode != WIN_INPUT_USED && (key == KEY_INS) &&
        (BitIsSet(state, KEY_STATE_UP)) && ThePlayerList &&
        ThePlayerList->getLocalPlayer()) {
      Player *localPlayer = ThePlayerList->getLocalPlayer();

      // Only toggle for human players
      if (localPlayer->getPlayerType() == PLAYER_HUMAN) {
        localPlayer->toggleAIAssist();
        returnCode = WIN_INPUT_USED;
      }
    }
