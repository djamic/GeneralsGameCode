...
AISkirmishPlayer::AISkirmishPlayer(Player *p)
    : AIPlayer(p), m_curFlankBaseDefense(0), m_curFrontBaseDefense(0),
      m_curFrontLeftDefenseAngle(0), m_curFrontRightDefenseAngle(0),
      m_curLeftFlankLeftDefenseAngle(0), m_curLeftFlankRightDefenseAngle(0),
      m_curRightFlankLeftDefenseAngle(0), m_curRightFlankRightDefenseAngle(0),
      m_frameToCheckEnemy(0), m_currentEnemy(NULL)
...
void AISkirmishPlayer::processBaseBuilding(void) {
  ...
  if (m_readyToBuildStructure) {
    DjLog("AISkirmishPlayer::processBaseBuilding - Entered for %s (Side: %s). "
          "Scanning build list...",
          m_player->getPlayerDisplayName().str(), m_player->getSide().str());
    ...
    for (BuildListInfo *info = m_player->getBuildList(); info;
      ...
      if (bldg == NULL) {
        // It was built before, but now gone?
          ...
          for (obj = TheGameLogic->getFirstObject(); obj;
            ...
            if (rhbi) {
              ...
              if (priorID == spawnerID) {
                DEBUG_LOG(
                    ("AI Found hole to rebuild %s", curPlan->getName().str()));
                info->setObjectID(obj->getID());
                bldg = obj; // Found the hole, treat as existing building (for
                            // now?)
                // Actually original code sets ID then loops again? No, it just
                // sets ID. If we found a hole, 'bldg' becomes the hole?
                // Original code: if hole found info->setObjectID.
                // Then next iteration it would find it?
                // No, we are in the loop.
                // If we set ID, next time `findObjectByID` works.
                // For this iteration, bldg is still NULL unless we set it.
                // Let's set bldg = obj;
              }
            }
          }
      ...
      if (bldg) {
        ...
        if (bldg->getControllingPlayer() == m_player) {
          ...
          if (bldg->getStatusBits().test(OBJECT_STATUS_UNDER_CONSTRUCTION)) {
            ...
            if (myDozer &&
                (myDozer->getControllingPlayer() != m_player ||
                 myDozer->isDisabledByType(
                     DISABLED_UNMANNED))) { // I don't expect this dozer
            ...
            if (myDozer == NULL) {
              DEBUG_LOG(("AI's Dozer got killed (or captured).  Find another "
                         "dozer."));
              queueDozer();
              myDozer = findDozer(bldg->getPosition());
              if (myDozer == NULL || myDozer->getAI() == NULL) {
                // DjLog("SKIP: Dozer lost and no replacement found for %s",
                // name.str());
                continue;
              }
        ...
        continue; // Already processed existing building
      }

      // Logic for Rebuild Delay
    ...
    if (powerInfo && powerPlan && !powerPlan->isEquivalentTo(bldgPlan)) {
      if (!powerUnderConstruction) {
        ...
        DjLog("AISkirmishPlayer::processBaseBuilding - Forcing Power Plant "
              "build");
      }
    }

    if (bldgPlan && bldgInfo) {
      DjLog("AISkirmishPlayer::processBaseBuilding - Attempting to build %s...",
      ...
      if (bldg) {
        ...
        DjLog("AISkirmishPlayer::processBaseBuilding - FAIL: "
              "buildStructureWithDozer returned NULL for %s",
              bldgPlan->getName().str());
      }
 ...
 * request and start training the unit. If busyOK is true, it will queue a
 * unit even if one is building.  This lets script invoked teams "push" to
...
void AISkirmishPlayer::acquireEnemy(void) {
  ...
  for (i = 0; i < ThePlayerList->getPlayerCount(); i++) {
    ...
    if (m_player->getRelationship(curPlayer->getDefaultTeam()) == ENEMIES) {
      ...
      // ok, we got an enemy;
      // If a player is out of units, or out of build facilities, we can
      // lower his priority.
      Bool inBadShape =
...
/**
   ...
   path. The first front defense is placed towards th Center path.  Number 2
   is placed to the left of #1, #3 is placed to the right of #1, #4 is
   placed to the left of #2 and so on.  So it looks like:

                                                                                                #1
                                                                         #2
   #3 #6  #4 #5 #7 #8 #9

        The flank base defenses cover the "Flank" approach, and the
   "Backdoor" approach. They alternate between these two, so the first flank
   defense covers flank, and the second covers backdoor, and continue to
   alternate.  They cover the approach using the same pattern as front
   above. John A.
...
// TheSuperHackers @info helmutbuhler 21/04/2025 This debug mutates the code to
// become CRC incompatible
#if defined(RTS_DEBUG) || !RETAIL_COMPATIBLE_CRC
    DEBUG_LOG(("buildAIBaseDefenseStructure -- Angle is %f sin %f, cos %f",
               180 * angle / PI, s, c));
    DEBUG_LOG(("buildAIBaseDefenseStructure -- Offset is %f  %f, Final "
               "Position is %f, %f",
               offset.x, offset.y, offset.x * c - offset.y * s,
...
/**
   ...
   dozer to fix, and returns true. If there is no bridge problem, returns
   false.
        */
Bool AISkirmishPlayer::checkBridges(Object *unit, Waypoint *way) {
  ...
  for (curWay = way; curWay; curWay = curWay->getNext()) {
    if (TheAI->pathfinder()->clientSafeQuickDoesPathExist(
            locoSet, &unitPos, curWay->getLocation())) {
      continue;
    }
        ...
        while (count > 0) {
          ...
          if (unit) {
            ...
            if (ai) {
              ...
              DEBUG_LOG(("Moving unit from %f,%f to %f,%f", pos.x, pos.y, to.x,
                         to.y));
#endif
              ai->aiMoveToPosition(
...
void AISkirmishPlayer::doBaseBuilding(void) {
  if (m_player->getCanBuildBase()) {
    ...
    // This timer is to keep from banging on the logic each frame.  If
    // something interesting happens, like a building is added or a unit
    // finished, the timers are shortcut.
    m_buildDelay--;
    if (m_buildDelay < 1) {
      if (m_readyToBuildStructure) {
        processBaseBuilding();
...
//----------------------------------------------------------------------------------------------------------
/**
 * Find any things that build stuff & add them to the build list.  Then
 * build any initially built buildings.
 */
void AISkirmishPlayer::newMap(void) {
  ...
  DjLog("AISkirmishPlayer::newMap - Loading BuildList for side: %s",
        mySide.str());
  DjLog("  BuildList source: TheAI->getAiData()->m_sideBuildLists");
  ...
  while (build) {
    if (build->m_side == mySide) {
      ...
      DjLog("AISkirmishPlayer::newMap - Loaded build list for side %s. "
            "List is "
            "%s",
            mySide.str(), (buildList ? "Valid" : "Empty/Null"));

      // Log each building and its AutomaticBuild status
      int buildingCount = 0;
      for (BuildListInfo *info = buildList; info; info = info->getNext()) {
        buildingCount++;
        DjLog("  Building #%d: %s (Template: %s) - AutoBuild: %s",
              buildingCount, info->getBuildingName().str(),
              info->getTemplateName().str(),
              info->isAutomaticBuild() ? "YES" : "NO");
      }
      DjLog("  Total buildings in list: %d", buildingCount);
  ...
  if (!build) {
    DjLog("AISkirmishPlayer::newMap - WARNING: No build list found for "
          "side %s",
          mySide.str());
  }
  ...
  for (BuildListInfo *info = m_player->getBuildList(); info;
    ...
    if (!bldgPlan) {
      DEBUG_LOG(
          ("*** ERROR - Build list building '%s' doesn't exist.", name.str()));
      continue;
    }
 ...
 */
Bool AISkirmishPlayer::computeSuperweaponTarget(
  ...
  if (power->getSpecialPowerType() == SPECIAL_CLUSTER_MINES ||
  ...
  return AIPlayer::computeSuperweaponTarget(power, retPos, playerNdx,
                                            weaponRadius);
}
