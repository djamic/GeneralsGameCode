...
AIPlayer::AIPlayer(Player *p)
    : m_player(p), m_buildDelay(0), m_teamDelay(0),
      ...
      m_readyToBuildTeam(false), m_readyToBuildStructure(false),
      m_structuresInQueue(0), m_repairDozer(INVALID_ID),
      m_skillsetSelector(INVALID_SKILLSET_SELECTION),
      m_dozerQueuedForRepair(false), m_supplySourceAttackCheckFrame(0),
      m_attackedSupplyCenter(INVALID_ID), m_teamSeconds(10),
...
void AIPlayer::onStructureProduced(Object *factory, Object *bldg) {
  ...
  for (info = m_player->getBuildList(); info; info = info->getNext()) {
    ...
    bldg->clearStatus(MAKE_OBJECT_STATUS_MASK2(OBJECT_STATUS_UNDER_CONSTRUCTION,
          ...
          for (obj = TheGameLogic->getFirstObject(); obj;
            ...
            if (rhbi) {
              ...
              if (priorID == spawnerID) {
                DEBUG_LOG(
                    ("AI Found hole to rebuild %s", bldgPlan->getName().str()));
...
Bool AIPlayer::computeSuperweaponTarget(const SpecialPowerTemplate *power,
  ...
  if (bounds.hi.x == 0 && bounds.lo.x == 0 && bounds.hi.y == 0 &&
      bounds.lo.y == 0) {
    Region3D bounds3D;
    // Degenerate bounds because he has no buildings.  Don't give up, scan the
    // whole map for the leftovers.
  ...
  if (power->getSpecialPowerType() == SPECIAL_SNEAK_ATTACK) {
    // Military units will have a negative effect on where to drop the special
    // power. We want to target rich areas that are poorly defended or
    // undefended.
    targetMilitaryUnits = FALSE;
  }

  // Randomize which way we iterate the grid. We don't always want to start in
  // the bottom left corner incase of a bad calculation, it'll would always end
  // up there.
    ...
    for (DLINK_ITERATOR<Team> iter = (*it)->iterate_TeamInstanceList();
      ...
      for (DLINK_ITERATOR<Object> iter = team->iterate_TeamMemberList();
        ...
        if (!includeMilitaryUnits) {
          if (pObj->isKindOf(KINDOF_FS_BASE_DEFENSE) ||
              pObj->isKindOf(KINDOF_TECH_BASE_DEFENSE)) {
            // Hostile structure
            applyNegValue = TRUE;
          } else if (pObj->isKindOf(KINDOF_VEHICLE) ||
                     pObj->isKindOf(KINDOF_INFANTRY)) {
            if (!pObj->isKindOf(KINDOF_DOZER) &&
                !pObj->isKindOf(KINDOF_HARVESTER)) {
              // Hostile unit.
              applyNegValue = TRUE;
            }
          }
        ...
        if (dx * dx + dy * dy < radSqr) {
          ...
          if (pObj->isKindOf(KINDOF_COMMANDCENTER)) {
            if (!includeMilitaryUnits)
              value =
                  value *
                  5.0f; // Command centers are prime targets for sneak attacks.
            else
              value =
                  value /
                  10; // Command centers cannot be killed by any superweapon, so
                      // we don't want to target them as highly. jba.
          }
          if (pObj->isKindOf(KINDOF_FS_SUPERWEAPON)) {
            if (!includeMilitaryUnits)
              value = value *
                      5.0f; // Superweapons are prime targets for sneak attacks.
            else
              value = value /
                      10; // Superweapons cannot be killed by any superweapon,
    ...
    if (factory) {
      ...
      for (Int j = 0; j < MAX_COMMANDS_PER_SET; j++) {
        ...
        if (commandButton->getName().isEmpty())
          continue;
        if (commandButton->getUpgradeTemplate() == NULL)
          continue;
...
void AIPlayer::buildBySupplies(Int minimumCash, const AsciiString &thingName) {
  ...
  if (bestSupplyWarehouse && tTemplate) {
    ...
    if (TheBuildAssistant->isLocationLegalToBuild(
      ...
      debugMessage.format(" %s - buildBySupplies unable to place near dock at "
                          "(%.2f,%.2f).  Attempting to adjust position.",
                          tTemplate->getName().str(), warehouseLocation->x,
                          warehouseLocation->y);
      ...
      for (posOffset = 0; posOffset < 2 * SUPPLY_CENTER_CLOSE_DIST;
        ...
        for (xPos = location.x - offset; xPos <= location.x + offset;
          ...
          if (TheGlobalData->m_debugSupplyCenterPlacement)
            DEBUG_LOG(
                ("buildBySupplies -- Fail at (%.2f,%.2f)", newPos.x, newPos.y));
          newPos.y = yPos + posOffset;
          valid = TheBuildAssistant->isLocationLegalToBuild(
                      &newPos, tTemplate, angle,
                      BuildAssistant::CLEAR_PATH |
    ...
    if (valid) {
      if (TheGlobalData->m_debugSupplyCenterPlacement)
        DEBUG_LOG(("buildAISupplyCenter -- SUCCESS at (%.2f,%.2f)", newPos.x,
                   newPos.y));
      location = newPos;
    }
    TheTerrainVisual->removeAllBibs(); // isLocationLegalToBuild adds bib
...
// ------------------------------------------------------------------------------------------------
Bool AIPlayer::calcClosestConstructionZoneLocation(
  ...
  Coord3D newPos = *location;
  if (TheBuildAssistant->isLocationLegalToBuild(
          location, constructTemplate, angle, BuildAssistant::NO_OBJECT_OVERLAP,
          NULL, m_player) != LBC_OK) {
    // Warn.
    AsciiString bldgName = constructTemplate->getName();
    bldgName.concat(" - calcClosestConstructionZoneLocation unable to place.  "
    ...
    for (posOffset = 0; posOffset < 2 * SUPPLY_CENTER_CLOSE_DIST;
      ...
      for (xPos = location->x - offset; xPos <= location->x + offset;
        ...
        newPos.y = yPos;
        valid = TheBuildAssistant->isLocationLegalToBuild(
                    &newPos, constructTemplate, angle,
                    BuildAssistant::CLEAR_PATH |
                        BuildAssistant::TERRAIN_RESTRICTIONS |
                        BuildAssistant::NO_OBJECT_OVERLAP,
                    NULL, m_player) == LBC_OK;
...
void AIPlayer::buildSpecificBuildingNearestTeam(const AsciiString &thingName,
  ...
  Coord3D newPos = *location;
  if (TheBuildAssistant->isLocationLegalToBuild(
          location, tTemplate, angle, BuildAssistant::NO_OBJECT_OVERLAP, NULL,
          m_player) != LBC_OK) {
    // Warn.
    AsciiString bldgName = tTemplate->getName();
    bldgName.concat(" - buildSpecificBuildingNearestTeam unable to place.  "
                    "Attempting to adjust position.");
    TheScriptEngine->AppendDebugMessage(bldgName, false);
    // try to fix.
    Real posOffset;
    ...
    for (DLINK_ITERATOR<TeamInQueue> iter = iterate_TeamReadyQueue();
      ...
      if (allIdle) {
        if (!team->m_sentToStartLocation) {
          ...
          /*
          if (team->m_team->getPrototype()->getTemplateInfo()->m_hasHomeLocation
          && !team->m_reinforcement) { AIGroupPtr theGroup =
          TheAI->createGroup(); if (theGroup) {
...
Object *AIPlayer::findDozer(const Coord3D *pos) {
  // DjLog("AIPlayer::findDozer called for pos %.2f, %.2f", pos->x, pos->y);
  ...
  if (needDozer) {
    DjLog("AIPlayer::findDozer - No suitable dozer found, queueing new one.");
    queueDozer();
  }
  if (closestDozer) {
    DjLog("AIPlayer::findDozer - returning closest dozer ID: %d",
          closestDozer->getID());
    return closestDozer;
  }
...
void AIPlayer::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    if (getFirstItemIn_TeamBuildQueue() != NULL) {
      ...
      DEBUG_CRASH(("AIPlayer::xfer - TeamBuildQueue head is not NULL, you "
                   "should delete it or something before loading a new list"));
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    if (getFirstItemIn_TeamReadyQueue() != NULL) {
      ...
      DEBUG_CRASH(("AIPlayer::xfer - TeamReadyQueue head is not NULL, you "
                   "should delete it or something before loading a new list"));
      throw SC_INVALID_DATA;
    }
...
void TeamInQueue::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    if (m_workOrders != NULL) {
      ...
      DEBUG_CRASH(("TeamInQueue::xfer - m_workOrders should be NULL but isn't. "
      ...
      for (DLINK_ITERATOR<Object> iter = team->iterate_TeamMemberList();
        ...
        if (pObj->isKindOf(KINDOF_STRUCTURE)) {
          if (conservative && pObj->isKindOf(KINDOF_CONSERVATIVE_BUILDING)) {
            // Kris - Aug 14, 2003
            // Conservative buildings (captured tech buildings, sneak attack
            // buildings are rejected). This was added so base boundaries aren't
            // potentially most of the map. Because sneak attack uses an inverse
            // cost calculation by avoiding defended areas, we need to keep
