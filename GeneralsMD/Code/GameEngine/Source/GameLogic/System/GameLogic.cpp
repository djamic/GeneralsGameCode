...
void GameLogic::startNewGame(Bool loadingSaveGame) {
      ...
      if (m_gameMode == GAME_SINGLE_PLAYER) {
        ...
        if (m_loadScreen) {
          TheWritableGlobalData->m_loadScreenRender =
              TRUE; ///< mark it so only a few select things are rendered during
                    ///< load
  ...
  TheWritableGlobalData->m_TiVOFastMode =
      FALSE; // always disable the TIVO fast-forward mode at the start of a new
             // game.
  ...
  // If forceFluffToProp == true, removable objects get created on client only.
  // [7/14/2003] If static lod is HIGH, we don't do force fluff to client side
  // only (create logic side props, more expensive. jba)
  ...
  if (TheGameLODManager->getStaticLODLevel() == STATIC_GAME_LOD_CUSTOM &&
    ...
    // Custom LOD, and volumetric shadows turned on - very high detail.  So use
    // logic props too. jba. [7/14/2003]
  ...
  if (TheRecorder && TheRecorder->isMultiplayer()) {
    useTrees = TRUE; // Always use trees in multiplayer, cause we want it to
                     // sync properly. jba.
    forceFluffToProp =
        TRUE; // Always do client side fluff - faster, and syncs properly. jba.
  }
    ...
    for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
      ...
      if (thingTemplate->isKindOf(KINDOF_CLEARED_BY_BUILD)) {
        if (thingTemplate->getFenceWidth() == 0.0f) {
          // Objects that are cleared by building, but aren't fences, are fluff.
          // jba. [7/14/2003]
          isFluff = true;
        }
      }
  ...
  if (TheGameInfo && !loadingSaveGame) {
    for (int i = 0; i < MAX_SLOTS; ++i) {
      ...
      if (slot->getPlayerTemplate() == PLAYERTEMPLATE_OBSERVER) {
        ...
        // Prevent from loading the disabled Generals, in case your game peer
        // hacked their GUI. The game will start, but the cheater will be
        // instantly defeated because he has no troops. This is also enforced at
        // GUI setup (GUIUtil.cpp and UserPreferences.cpp).
        // @todo: unlock these when something rad happens

        // TheSuperHackers @logic-client-separation helmutbuhler 11/04/2025
        // TheChallengeGenerals belongs to client, we shouldn't depend on that
        // here.
        Bool disallowLockedGenerals = TRUE;
        const GeneralPersona *general =
            TheChallengeGenerals->getGeneralByTemplateName(pt->getName());
        Bool startsLocked = general ? !general->isStartingEnabled() : FALSE;
        if (disallowLockedGenerals && startsLocked)
          continue;

        // prevent from loading disallowed templates, in case your peer hacked
        // their GUI.

        // So that the global flag for restricting factions to "OLD" is applied
        // only in the appropriate context! Trouble was that skirmish games
        // would get no command centers upon start, if this was set true in a
        // GameSpyMenu
        if (isInInternetGame()) {
  ...
  if (isChallengeCampaign) {
    ...
    DEBUG_ASSERTCRASH(placeholderThePlayer,
    ...
    if (placeholderThePlayer) {
      ...
      while (thePlayerEnemysMask) {
        // set local player enemy relationships based on "ThePlayer" enemy
        // relationships
...
// If defined, the game times various units.
  ...
  if (loadingSaveGame == FALSE) {
    // Drawables need to do some work on level start; give them a chance to do
    // it
  ...
  if (isInReplayGame() && TheInGameUI && TheGameText) {
    TheInGameUI->messageNoFormat(TheGameText->FETCH_OR_SUBSTITUTE(
        "GUI:FastForwardInstructions", L"Press F to toggle Fast Forward"));
  }
}

//-----------------------------------------------------------------------------------------
void GameLogic::createOptimizedTree(const ThingTemplate *thingTemplate,
                                    Coord3D *pos, Real angle) {
  // Opt trees and props just get drawables to tell the client about it, then
  // deleted. jba [6/5/2003] This way there is no logic object to slow down
  // partition manager and core logic stuff. TheSuperHackers @info Destroying
  // the drawable will register the tree in the tree buffer.
  ...
  if (m_shouldValidateCRCs && !TheNetwork->sawCRCMismatch()) {
    ...
    if (TheNetwork) {
      ...
      if (m_cachedCRCs.size() < numPlayers) {
        ...
        // DEBUG_LOG(("Comparing %d CRCs on frame %d", m_cachedCRCs.size(),
        // m_frame));
        std::map<Int, UnsignedInt>::const_iterator crcIt = m_cachedCRCs.begin();
        Int validatorCRC = crcIt->second;
        // DEBUG_LOG(("Validator CRC from player %d is %8.8X", crcIt->first,
        // validatorCRC));
...
void GameLogic::selectObject(Object *obj, Bool createNewSelection,
  ...
  if (!obj->isMassSelectable() && !createNewSelection) {
    DEBUG_LOG(("GameLogic::selectObject() - Object attempted to be added to "
...
// version present for clarity. (Yes, this is worth doing.) (srj)
  ...
  UpdateModulePtr *pSZ =
      &m_sleepyUpdates[0] + m_sleepyUpdates.size(); // yes, this is off the end.
...
UpdateModulePtr GameLogic::peekSleepyUpdate() const {
  ...
  DEBUG_ASSERTCRASH(
      u->friend_getIndexInLogic() == 0,
      ("index mismatch: expected %d, got %d", 0, u->friend_getIndexInLogic()));
  return u;
}
...
#define DO_SINGLE_KIND
  ...
  if (timeFrames > 0) {
    ...
    if (mode == LOGIC) {
      ...
      drawCallLogic =
          (float)drawCallTotal /
          (float)(TIME_FRAMES * TOTAL_UNITS); // 100 units for TIME_FRAMES
    ...
    sprintf(remark, "All %f: (%d ms) for %d %s's\n", timeAll,
            (Int)(timeAll * 1000 / TIME_FRAMES), TOTAL_UNITS, thingName.str());
    DEBUG_LOG((remark));
...
#define dont_DO_ATTACK
      ...
      attackedPos.z =
          TheTerrainLogic->getGroundHeight(attackedPos.x, attackedPos.y);
...
void GameLogic::update(void) {
  ...
  if (generateForSolo || generateForMP) {
    ...
    // During replay simulation, we bypass TheMessageStream and instead put the
    // CRC message directly into TheCommandList because we don't update
    // TheMessageStream during simulation.
...
void GameLogic::addObjectToLookupTable(Object *obj) {
  ...
  while (newID >= m_objVector.size()) // Fail case is hella rare, so faster to
...
void GameLogic::removeObjectFromLookupTable(Object *obj) {
  ...
  // sanity
  // TheSuperHackers @fix Mauller/Xezon 24/04/2025 Prevent out of range access
  // to vector lookup table
...
UnsignedInt GameLogic::getCRC(Int mode, AsciiString deepCRCFileName) {
...
#ifdef DEBUG_CRC
    // TheSuperHackers @info helmutbuhler 04/09/2025
    // This allows you to save the binary data that is involved in the crc
    // calculation to a binary file per frame. This was apparently used early in
    // development and isn't that useful, because diffing that binary data is
    // very difficult. The CRC logging is much easier to diff and also more
  ...
  if (isInGameLogicUpdate()) {
    CRCGEN_LOG(
        ("CRC at start of frame %d is 0x%8.8X", m_frame, xferCRC->getCRC()));
  }
  ...
  if (isInGameLogicUpdate()) {
    CRCGEN_LOG(("CRC after partition manager for frame %d is 0x%8.8X", m_frame,
                xferCRC->getCRC()));
  }
  ...
  if ((g_crcModuleDataFromClient && !isInGameLogicUpdate()) ||
    ...
    if (isInGameLogicUpdate()) {
      CRCGEN_LOG(("CRC after module factory for frame %d is 0x%8.8X", m_frame,
                  xferCRC->getCRC()));
    }
  }
#endif // DEBUG_CRC
  ...
  if (isInGameLogicUpdate()) {
    CRCGEN_LOG(("CRC after PlayerList for frame %d is 0x%8.8X", m_frame,
                xferCRC->getCRC()));
  }
  ...
  if (isInGameLogicUpdate()) {
    CRCGEN_LOG(
        ("CRC after AI for frame %d is 0x%8.8X", m_frame, xferCRC->getCRC()));
  }
...
void GameLogic::exitGame() {
  // TheSuperHackers @fix The logic update must not be halted to process the
  // game exit message.
  setGamePaused(FALSE);
...
void GameLogic::processProgressComplete(Int playerId) {
  ...
  if (m_progressComplete[playerId] == TRUE) {
    DEBUG_LOG(("GameLogic::processProgressComplete, playerId %d is marked TRUE "
 ...
 * will forget your exp 10: xfer m_superweaponRestriction
 */
// ------------------------------------------------------------------------------------------------
void GameLogic::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    for (UnsignedInt i = 0; i < objectCount; ++i) {
      ...
      if (tocEntry == NULL) {
        ...
        DEBUG_CRASH(
            ("GameLogic::xfer - No TOC entry match for id '%d'", tocID));
        throw SC_INVALID_DATA;
      }
  ...
  if (version >= 3) {
    ...
    if (sanityTriggerCount != triggerCount) {
      ...
      DEBUG_CRASH(("GameLogic::xfer - Polygon trigger count mismatch.  Save "
  ...
  if (version >= 7) {
    if (xfer->getXferMode() == XFER_SAVE) {
      ...
      if (m_thingTemplateBuildableOverrides.empty() == false) {
        DEBUG_CRASH(("GameLogic::xfer - m_thingTemplateBuildableOverrides "
  ...
  if (version >= 8) {
    ...
    if (xfer->getXferMode() == XFER_SAVE) {
      ...
      if (m_controlBarOverrides.empty() == false) {
        DEBUG_CRASH(("GameLogic::xfer - m_controlBarOverrides should be empty, "
                     "but is not"));
        throw SC_INVALID_DATA;
      }
