...
extern void externalAddTree(Coord3D location, Real scale, Real angle,
                            AsciiString name);
#endif
...
void GameLogic::destroyAllObjectsImmediate() {
  ...
  // TheSuperHackers @bugfix xezon 22/05/2025 Set all remaining objects
  // effectively dead to avoid triggering their death modules that eventually
  // would spawn new objects, such as debris, which could then crash the game.
...
void GameLogic::reset(void) {
  ...
  WeatherSetting *ws =
      (WeatherSetting *)TheWeatherSetting.getNonOverloadedPointer();
...
static Object *placeObjectAtPosition(Int slotNum,
  ...
  DEBUG_ASSERTCRASH(
      btt,
      ("TheThingFactory didn't find a template in placeObjectAtPosition()"));
...
static void placeNetworkBuildingsForPlayer(Int slotNum, const GameSlot *pSlot,
  ...
  DEBUG_ASSERTCRASH(
      ...
      ("no starting building type for player %d (playertemplate %ls)", slotNum,
       pTemplate->getDisplayName().str()));
...
#define MORE_RANDOM
  ...
  for (i = 0; i < ThePlayerTemplateStore->getPlayerTemplateCount(); ++i) {
    ...
    const GeneralPersona *general =
        TheChallengeGenerals->getGeneralByTemplateName(ptTest->getName());
  ...
  for (i = 0; i < MAX_SLOTS; ++i) {
    ...
    while (playerTemplateIdx != PLAYERTEMPLATE_OBSERVER &&
      ...
      DEBUG_ASSERTCRASH(playerTemplateIdx == PLAYERTEMPLATE_RANDOM,
    ...
    if (colorIdx < 0 || colorIdx >= TheMultiplayerSettings->getNumColors()) {
      DEBUG_ASSERTCRASH(colorIdx == -1,
                        ("Non-random bad color %d in slot %d", colorIdx, i));
      while (colorIdx == -1) {
        colorIdx =
...
static void populateRandomStartPosition(GameInfo *game) {
  ...
  for (i = 0; i < MAX_SLOTS; ++i) {
    ...
    DEBUG_ASSERTCRASH(
        posIdx == -1,
        ("Non-random bad start position %d in slot %d", posIdx, i));

    // choose a starting position
    Int team = slot->getTeamNumber();
    if (!hasStartSpotBeenPicked) { // We're the first real spot.  Pick randomly.
      while (posIdx == -1) { // This while loop shouldn't be neccessary, since
                             // we're first.  Why not, though?
        posIdx = GameLogicRandomValue(0, numPlayers - 1);
        if (game->isStartPositionTaken(posIdx))
          posIdx = -1;
      }
    ...
    } else { // pick teams far apart, team members close together
      if (team < 0 ||
          teamPosIdx[team] == -1) // if team None or team not yet placed
      {                           // pick position furthest from all other teams
        ...
        for (posIdx = 0; posIdx < numPlayers; ++posIdx) {
          ...
          if (farthestIndex < 0) { // take this one as best if none else
            farthestIndex = posIdx;
            for (Int n = 0; n < numPlayers; ++n) {
              if (taken[n] && n != posIdx)
                farthestDistance += startSpotDistance[posIdx][n];
            }
          } else { // find empty position furthest from all taken positions
            Real dist = 0.0f;
            for (Int n = 0; n < numPlayers; ++n) {
              if (taken[n] && n != posIdx)
                dist += startSpotDistance[posIdx][n];
      ...
      {      // pick position closest to team
        ...
        for (Int n = 0; n < numPlayers; ++n) {
          ...
          if (!taken[n] && dist < closestDist) { // found a better match
            closestDist = dist;
            closestIdx = n;
          }
        }
        DEBUG_ASSERTCRASH(closestDist < FLT_MAX,
                          ("Couldn't find a closest starting positon!"));
...
<<<<<<< HEAD
      ...
      == == == = if (m_startNewGame == FALSE) {
        ...
        /// movie or something.
        // Failing that, we just set the flag so the actual game can start from
        // a uniform entry point (startNewGame() called from update()).
    ...
    // save game. for save games, we read this value out of the save game file
    // and it is important that we preserve it as we load and execute the game
    //
    if (loadingSaveGame == FALSE)
      m_nextObjID = (ObjectID)1;
    ...
    TheWritableGlobalData->m_TiVOFastMode =
        FALSE; // always disable the TIVO fast-forward mode at the start of a
               // new game.

    Campaign *currentCampaign = TheCampaignManager->getCurrentCampaign();
    Bool isChallengeCampaign = m_gameMode == GAME_SINGLE_PLAYER &&
                               currentCampaign &&
                               currentCampaign->m_isChallengeCampaign;
    ...
    // On a NEW game, we need to copy the superweapon restrictions from the game
    // info to here (because TheGameInfo is not always saved and doesn't carry
    // over to replays). On a save game, we save the superweapon restrictions in
    // GameLogic::xfer()
    if (!loadingSaveGame) {
      if (TheGameInfo) {
...
<<<<<<< HEAD
    ...
    if (!m_loadScreen &&
        !(TheRecorder &&
          TheRecorder->getMode() == RECORDERMODETYPE_SIMULATION_PLAYBACK)) {
      m_loadScreen = getLoadScreen(loadingSaveGame);
      if (m_loadScreen) {
    ...
    if (TheGameInfo) {
      ...
      for (int i = 0; i < MAX_SLOTS; ++i) {
        ...
        DEBUG_LOG(("Player %d's teams are: allies=%s, enemies=%s", i,
        ...
        /*
           ...
           TheMultiplayerSettings->getNumColors())
                                {
                                        DEBUG_ASSERTCRASH(colorIdx == -1,
           ("Non-random bad color %d in slot %d", colorIdx, i)); while (colorIdx
           == -1)
                                        {
                                                colorIdx =
           GameLogicRandomValue(0, TheMultiplayerSettings->getNumColors()-1); if
           (game->isColorTaken(colorIdx)) colorIdx = -1;
                                        }
                                        DEBUG_LOG(("Setting color %d to %d", i,
        ...
        d.setInt(TheKey_playerNightColor,
                 TheMultiplayerSettings->getColor(slot->getColor())
                     ->getNightColor());
        ...
        d.setBool(TheKey_multiplayerIsLocal,
                  slot->isHuman() &&
                      (slot->getName().compare(
                           TheGameInfo->getSlot(TheGameInfo->getLocalSlotNum())
                               ->getName()
                               .str()) == 0));
        ...
        */
        ...
        if (slot->isHuman() && TheGameInfo->getSlotNum(slotNameAscii) ==
    ...
    d.setInt(TheKey_playerColor,
             TheMultiplayerSettings->getColor(0)->getColor());
    d.setInt(TheKey_playerNightColor,
    ...
    if (TheGameEngine->isMultiplayerSession() || isSkirmishOrSkirmishReplay) {
      ...
      {
              AsciiString name;
              name.format("MultiplayerPlayerDefeat"); // name is for the ui in
      the editor Script *pNewScript = newInstance(Script);
    ...
    Player *observerPlayer = ThePlayerList->findPlayerWithNameKey(
    ...
    for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
      ...
      if (pMapObj->getFlag(FLAG_BRIDGE_FLAGS) ||
        ...
        continue; // these roads & bridges are special cased in the terrain
                  // side.
    ...
    DEBUG_LOG(("Reveal shroud for %ls whose index is %d",
               observerPlayer->getPlayerDisplayName().str(),
               observerPlayer->getPlayerIndex()));
    ...
    // If forceFluffToProp == true, removable objects get created on client
    // only. [7/14/2003] If static lod is HIGH, we don't do force fluff to
    // client side only (create logic side props, more expensive. jba)
    Bool forceFluffToProp =
        TheGameLODManager->getStaticLODLevel() < STATIC_GAME_LOD_HIGH;
    if (TheGameLODManager->getStaticLODLevel() == STATIC_GAME_LOD_CUSTOM &&
        TheGlobalData->m_useShadowVolumes) {
      // Custom LOD, and volumetric shadows turned on - very high detail.  So
      // use logic props too. jba. [7/14/2003]
      forceFluffToProp = false;
    }

    if (TheRecorder && TheRecorder->isMultiplayer()) {
      useTrees = TRUE; // Always use trees in multiplayer, cause we want it to
                       // sync properly. jba.
      forceFluffToProp = TRUE; // Always do client side fluff - faster, and
                               // syncs properly. jba.
    }
    ...
    if (loadingSaveGame) {
      // Loading a loadingSaveGame, need to add the trees to the client. jba.
      // [8/11/2003]
      for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
           pMapObj = pMapObj->getNext()) {
        // get thing template based from map object name
      ...
      for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
        ...
        // have a test string (designated with *** and the define TEST_STRING)
        // and will have temporary templates created 'on the fly' during a
        ...
        if (thingTemplate->isKindOf(KINDOF_CLEARED_BY_BUILD)) {
          if (thingTemplate->getFenceWidth() == 0.0f) {
            // Objects that are cleared by building, but aren't fences, are
            // fluff. jba. [7/14/2003]
            isFluff = true;
          }
        }
...
>>>>>>> upstream/main
        ...
        if (game) {
          ...
          if (TheGameEngine->isMultiplayerSession() ||
              isSkirmishOrSkirmishReplay) {
            // Saves off any player, and resets the sides to 0 players so we can
            // add the skirmish players.
            TheSidesList->prepareForMP_or_Skirmish();
          }
...
<<<<<<< HEAD
      ...
      >>>>>>> upstream/main
               ...
               TheMultiplayerSettings->getNumColors())
                                    {
                                            DEBUG_ASSERTCRASH(colorIdx == -1,
               ("Non-random bad color %d in slot %d", colorIdx, i)); while
      (colorIdx
               == -1)
                                            {
                                                    colorIdx =
      ...
      i, colorIdx)); slot->setColor(colorIdx);
                                    }
                                    */
            ...
            d.setBool(TheKey_multiplayerIsLocal,
                      slot->isHuman() &&
                          (slot->getName().compare(
                               game->getSlot(game->getLocalSlotNum())
                                   ->getName()
                                   .str()) == 0));
...
<<<<<<< HEAD
        ...
        if (TheGameEngine->isMultiplayerSession() ||
            isSkirmishOrSkirmishReplay) {
          // if there are no other teams (happens for debugging) don't end the
          // game immediately
          Int numTeams = 0; // this can be higher than expected, but is accurate
                            // for determining 0, 1, 2+
          Int lastTeam = -1;
          if (game) {
            for (int i = 0; i < MAX_SLOTS; ++i) {
              const GameSlot *slot = game->getConstSlot(i);
          ...
          if (numTeams > 1) {
            ...
            if (theInputStream.open(path)) {
              ChunkInputStream *pStrm = &theInputStream;
              DataChunkInput file(pStrm);
              file.registerParser("PlayerScriptsList",
          ...
          {
                  AsciiString name;
                  name.format("MultiplayerPlayerDefeat"); // name is for the ui
          in the editor Script *pNewScript = newInstance(Script);
          pNewScript->setName(name);

                  Condition *pFalse1 =
          newInstance(Condition)(Condition::MULTIPLAYER_PLAYER_DEFEAT);
          OrCondition *pOr = newInstance(OrCondition);
          pOr->setFirstAndCondition(pFalse1); pNewScript->setOrCondition(pOr);
          ...
          newInstance(ScriptAction)(ScriptAction::LOCALDEFEAT);
                  ...
                  ScriptList *pSL =
          TheSidesList->getSideInfo(0)->getScriptList();
                  pSL->addScript(pNewScript, 0); // adds this as the player's
          ...
          {
                  AsciiString name;
                  name.format("MultiplayerVictory"); // name is for the ui in
          the editor Script *pNewScript = newInstance( Script );
                  pNewScript->setName(name);

                  Condition *pFalse1 =
          newInstance(Condition)(Condition::MULTIPLAYER_ALLIED_VICTORY);
          OrCondition *pOr = newInstance(OrCondition);
          pOr->setFirstAndCondition(pFalse1); pNewScript->setOrCondition(pOr);
          ...
          pNewScript->setName(name);
                  ...
                  Condition *pFalse1 =
          newInstance(Condition)(Condition::MULTIPLAYER_ALLIED_DEFEAT);
          OrCondition *pOr = newInstance(OrCondition);
          pOr->setFirstAndCondition(pFalse1); pNewScript->setOrCondition(pOr);
        ...
        ThePartitionManager->init();
        ThePartitionManager
            ->refreshShroudForLocalPlayer(); // Can't do this until after init,
                                             // and doesn't seem right to do in
                                             // init
        ...
        GetPrecisionTimer(&endTime64);
        sprintf(
            Buf, "After terrainlogic->newmap=%f",
            ((double)(endTime64 - startTime64) / (double)(freq64) * 1000.0));
        DEBUG_LOG(("%s", Buf));
#endif
        ...
        for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
          ...
          if (obj) {
            ...
            // Do this after positioning the object, because we may place
            // objects in response to keys in the map object properties. update
            // this object instance with properties from the map object
            obj->updateObjValuesFromMapProperties(pMapObj->getProperties());
          }
        }
        ...
        // tell the AI about it
        // Note that it is important that the pathfinder be called before the
        // map objects are loaded.
        TheAI->pathfinder()->newMap();
        ...
        Int timer = timeGetTime();
        if (loadingSaveGame) {
          // Loading a loadingSaveGame, need to add the trees to the client.
          // jba. [8/11/2003]
          for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
               pMapObj = pMapObj->getNext()) {
            // get thing template based from map object name
            ...
            if (thingTemplate == NULL)
              continue;
            // don't create trees and shrubs if this is one and we have that
            // option off
            if (thingTemplate->isKindOf(KINDOF_SHRUBBERY) && !useTrees)
              continue;
          ...
          for (pMapObj = MapObject::getFirstMapObject(); pMapObj;
            ...
            if (pMapObj->getFlag(FLAG_BRIDGE_FLAGS) ||
                pMapObj->getFlag(FLAG_ROAD_FLAGS)) {
              continue; // roads & bridges are special cased in the terrain
                        // side.
            }
            extern Int DX8Wrapper_PreserveFPU;

            // Kris: Added this function so that we can rename objects and
            // preserve them. If the patch
            //       entry is missing for a particular unit, it doesn't get
            //       added to the world.
            handleNameChange(pMapObj);
            ...
            //
            // if no template continue, some map objects don't have thing
            // templates like lights (handled in the device).  Objects that are
            // test objects have a test string (designated with *** and the
            // define TEST_STRING) and will have temporary templates created 'on
            // the fly' during a ThingFactory->findTemplate() call when loading
            // from the map file
            //
            if (thingTemplate == NULL)
              continue;
            ...
            if (obj) {
              ...
              // Do this after positioning the object, because we may place
              // objects in response to keys in the map object properties.
              // update this object instance with properties from the map object
              obj->updateObjValuesFromMapProperties(pMapObj->getProperties());

              PathfindLayerEnum layer =
                  TheTerrainLogic->getLayerForDestination(&pos);
              obj->setLayer(layer);
              ...
              for (BehaviorModule **m = obj->getBehaviorModules(); *m; ++m) {
                ...
                create->onBuildComplete();
                == == == =
                             // So that the global flag for restricting factions
                             // to "OLD" is applied only in the appropriate
                             // context! Trouble was that skirmish games would
                             // get no command centers upon start, if this was
                             // set true in a GameSpyMenu
                    if (isInInternetGame()) {
          ...
          if (game && !loadingSaveGame) {
            for (int i = 0; i < MAX_SLOTS; ++i) {
              ...
              if (slot->getPlayerTemplate() == PLAYERTEMPLATE_OBSERVER) {
                ...
                if (pt) {
                  for (Int j = 0;
                       j < ThePlayerTemplateStore->getPlayerTemplateCount();
                       ++j) {
                ...
                // Prevent from loading the disabled Generals, in case your game
                // peer hacked their GUI. The game will start, but the cheater
                // will be instantly defeated because he has no troops. This is
                // also enforced at GUI setup (GUIUtil.cpp and
                // UserPreferences.cpp).
                // @todo: unlock these when something rad happens

                // TheSuperHackers @logic-client-separation helmutbuhler
                // 11/04/2025 TheChallengeGenerals belongs to client, we
                // shouldn't depend on that here.
                Bool disallowLockedGenerals = TRUE;
                const GeneralPersona *general =
                    TheChallengeGenerals->getGeneralByTemplateName(
                        pt->getName());
                Bool startsLocked =
                    general ? !general->isStartingEnabled() : FALSE;
                if (disallowLockedGenerals && startsLocked)
                  continue;

                // prevent from loading disallowed templates, in case your peer
                // hacked their GUI.

                // So that the global flag for restricting factions to "OLD" is
                // applied only in the appropriate context! Trouble was that
                // skirmish games would get no command centers upon start, if
                // this was set true in a GameSpyMenu
                if (isInInternetGame()) {
...
<<<<<<< HEAD
              ...
              == == == =
                           // Note - WorldBuilderDoc.cpp also uses initial
                           // camera position, so if changed, update both.  jba
                           // Note - We construct the multiplayer start spot
              ...
              if (TheGameInfo) {
                ...
                DEBUG_ASSERTCRASH(slot,
                                  ("Starting a LAN game without ourselves!"));
>>>>>>> upstream/main
            ...
            //
            // tell the client to pre-load some assets that we will use such as
            // faction things we will build and various damage states for all
            // the structures on the map so that we don't have big pauses when
            // building those objects or switching to those states
            //
            ...
            // Note - WorldBuilderDoc.cpp also uses initial camera position, so
            // if changed, update both.  jba Note - We construct the multiplayer
            // start spot name manually here, so change this if you
            //        change TheKey_Player_1_Start etc.  mdc
            ...
            if (game) {
              ...
              if (slot->isHuman()) {
                ...
                startingCamName.format(
                    "Player_%d_Start",
                    startPos + 1); // start pos waypoints are 1-based
                DEBUG_LOG(
                    ("Using %s as the multiplayer initial camera position",
                     startingCamName.str()));
              }
            }
            ...
            if (way) {
              ...
              // Just look somewhere.  lookAt does some terrain specific setup,
              // so it is good to call it.  jba
              Coord3D pos;
              pos.x = 50;
              pos.y = 50;
              pos.z = 0;
            ...
            // update partition info - We need to do the initial update so that
            // it can be queried during the first frame.  jba.
            ThePartitionManager->update();
            ...
            if (isChallengeCampaign) {
              ...
              // Some designers have used "ThePlayer" as an empty player for
              // Generals' Challenge maps which they reference in script and by
              // which they set player relationships.  The skirmish local player
              // will receive these relationships.  If there is no "ThePlayer",
              // we have to assume that all players on the map are the local
              // player's enemy, except neutral and civilian players.
              DEBUG_ASSERTCRASH(
                  localPlayer,
                  ("Local player has not been established for Challenge map."));
              Player *placeholderThePlayer =
                  ThePlayerList->findPlayerWithNameKey(NAMEKEY("ThePlayer"));
              DEBUG_ASSERTCRASH(
                  placeholderThePlayer,
                  ("Challenge maps without player \"ThePlayer\" assume "
                   "that the local player is mutual enemies with all other "
                   "players except the neutral and civilian players."));

              if (placeholderThePlayer) {
                PlayerMaskType thePlayerEnemysMask =
                    ThePlayerList->getPlayersWithRelationship(
                        placeholderThePlayer->getPlayerIndex(), ALLOW_ENEMIES);
                while (thePlayerEnemysMask) {
                  // set local player enemy relationships based on "ThePlayer"
                  // enemy relationships
                  Player *thePlayerEnemy =
                      ThePlayerList->getEachPlayerFromMask(thePlayerEnemysMask);
                  thePlayerEnemy->setPlayerRelationship(localPlayer, ENEMIES);
                  localPlayer->setPlayerRelationship(thePlayerEnemy, ENEMIES);
                }
              } else {
                // This map has no "ThePlayer" by which to model the local
                // player's relationships, so make assumptions.
                for (Int i = 0; i < ThePlayerList->getPlayerCount(); i++) {
                  ...
                  if (thatPlayer == localPlayer) {
                    rel = ALLIES;
                             ...
                             thatPlayer != ThePlayerList->findPlayerWithNameKey(
                                               NAMEKEY("PlyrCivilian"))) {
                    rel = ENEMIES;
                  }
            ...
            if (m_loadScreen) {
              ...
              /*
                              //
                              // delete load screen only when not loading a save
                 game, for save games we still
                              // have more work to do and the load screen will
                 be deleted elsewhere after
...
// If defined, the game times various units.
            ...
            fputs("Full,100*ms,NoPart-NoSpawn,,No "
                  "Spawn,100*ms,Logic,100*ms,Thing,Model,Kind,Side,DrawCalls "
                  "All,DrawCalls NoPart-NoSpawn,DrawCalls NoSpawn\n",
                  g_UT_commaLog);
            ...
            // Just look somewhere.  lookAt does some terrain specific setup, so
            // it is good to call it.  jba
            Coord3D thePos;
            thePos.x = 50;
            thePos.y = 50;
            thePos.z = 0;
            ...
            // If we are now starting a multiplayer or skirmish game, let us set
            // the local players selectionto be the command center We'll ask the
            // Recorder, so we survive replays
            if (TheRecorder->isMultiplayer()) {
              ...
              for (Int i = 0; i < MAX_PLAYER_COUNT; ++i) {
                ...
                if (player && player->isPlayerActive()) {
                  // we need to iterate their objects, and select the first
                  // Command Center we find
                  Bool alreadyFound = FALSE;
                  player->iterateObjects(findAndSelectCommandCenter,
                                         &alreadyFound);
                }
              }
            }
            ...
            if (TheGameSpyBuddyMessageQueue && TheGameSpyGame &&
                isInInternetGame()) {
              BuddyRequest req;
              ...
              sprintf(
                  ...
                  WideCharStringToMultiByte(TheGameSpyGame->getGameName().str())
                      .c_str());
              TheGameSpyBuddyMessageQueue->addRequest(req);
            }

            if (loadingSaveGame == FALSE) {
              // Drawables need to do some work on level start; give them a
              // chance to do it
            ...
            // Assume that getting this far means we've successfully entered an
            // online game. Add an additional disconnection to player stats in
            // case he doesn't complete this game. -MW
            if (TheGameSpyInfo)
            ...
            if (isInReplayGame() && TheInGameUI && TheGameText) {
              TheInGameUI->messageNoFormat(TheGameText->FETCH_OR_SUBSTITUTE(
                  ...
                  L"Press F to toggle Fast Forward"));
            }
          }

          //-----------------------------------------------------------------------------------------
          void GameLogic::createOptimizedTree(
              const ThingTemplate *thingTemplate, Coord3D *pos, Real angle) {
            // Opt trees and props just get drawables to tell the client about
            // it, then deleted. jba [6/5/2003] This way there is no logic
            // object to slow down partition manager and core logic stuff.
            // TheSuperHackers @info Destroying the drawable will register the
            // tree in the tree buffer.
          ...
          void GameLogic::loadMapINI(AsciiString mapName) {
            ...
            //
            // if map name begins with a "SAVE_DIRECTORY\", then the map refers
            // to a map that has been extracted from a save game file ... in
            // that case we need to get the pristine map name string in order to
            // manipulate and load the right map.ini for that map from it's
            // original location
            //
            const char *pristineMapName =
                TheGameState->isInSaveDirectory(mapName.str())
            ...
            // back up over the ".map" extension and to the first directory
            // separator
            ...
            *extension = 0;
            ...
            if (TheFileSystem->doesFileExist(fullFledgeFilename)) {
              ...
              INI ini;
              ini.load(AsciiString(fullFledgeFilename),
                       INI_LOAD_CREATE_OVERRIDES, NULL);
            }

            // TheSuperHackers @todo Implement ini load directory for map
            // folder. Requires adjustments in map transfer.
            ...
            if (TheDisplay) {
              ...
              sprintf(fullFledgeFilename, "%s\\%s", filename,
                      ASSET_USAGE_FILE_NAME);
          ...
          void GameLogic::processDestroyList(void) {
            ...
            for (ObjectPointerListIterator iterator =
                     m_objectsToDestroy.begin();
                 iterator != m_objectsToDestroy.end(); iterator++) {
              Object *currentObject = (*iterator);

#ifdef ALLOW_NONSLEEPY_UPDATES
              for (std::list<UpdateModulePtr>::iterator it =
              ...
              /*
                      this looks odd, but is necessary; since erasing a single
                 entry can shuffle others in the list (in order to maintain its
                 heap-ness), we must do two passes: one to find the updates for
                 this object, another to actually erase 'em.

                      (in case you're wondering: yes, this is still more
                 efficient than just deleting them and rebalancing the entire
                 heap afterwards, at least for real-world maps, since an
                 individual rebalance is O(log N) and a full rebalance is
                 O(N)... so unless you are deleting the majority of the objects
                 in the world every frame, we come out well ahead this way.)
              */
              ...
              for (std::vector<UpdateModulePtr>::iterator it2 =
                       m_sleepyUpdates.begin();
                   it2 != m_sleepyUpdates.end(); ++it2) {
                UpdateModulePtr u = *it2;
                if (u->friend_getObject() == currentObject &&
                    numSUO < MAX_SUO) {
              ...
              for (--numSUO; numSUO >= 0; --numSUO) {
                // have to re-get idx each time since each call to erase might
                // change others.
                Int idx = sleepyUpdatesForThisObject[numSUO]
                              ->friend_getIndexInLogic();
                DEBUG_ASSERTCRASH(m_sleepyUpdates[idx] ==
                                      sleepyUpdatesForThisObject[numSUO],
                                  ("Hmm, expected update mismatch here"));
                eraseSleepyUpdate(idx);
                DEBUG_ASSERTCRASH(sleepyUpdatesForThisObject[numSUO]
                                          ->friend_getIndexInLogic() == -1,
                                  ("Hmm, expected index to be -1 here"));
              }

              currentObject->removeFromList(
                  &m_objList); // remove from object list
            ...
            m_objectsToDestroy.clear(); // list full of bad pointers now, clear
                                        // it.  If anyone's deletion resulted
            // in the request for a new deletion (sub-object), the new object
            // was added to the end of this list.
          }
        ...
        group->removeAll();
          ...
          // ------------------------------------------------------------------------------------------------
          // ------------------------------------------------------------------------------------------------
          void GameLogic::deselectObject(
              Object * obj, PlayerMaskType playerMask, Bool affectClient) {
            if (!obj) {
              return;
            }
            ...
            while (playerMask) {
              ...
              CRCGEN_LOG(("Removing a unit from a selected group in "
...
// pretty slow, so do only for DEBUG_CRASHING for now. turn on if you suspect
// wonkiness.
#ifdef DEBUG_CRASHING
#define SLEEPY_DEBUG
#endif
            ...
            //%08lx",i,m_sleepyUpdates[i],m_sleepyUpdates[i]->friend_getNextCallFrame()));
            // }
            for (i = 0; i < sz; ++i) {
              DEBUG_ASSERTCRASH(m_sleepyUpdates[i]->friend_getIndexInLogic() ==
                                    i,
                                ("index mismatch: expected %d, got %d", i,
              ...
              if (i > 0) {
                ...
                DEBUG_ASSERTCRASH(pri >= pri0,
                                  ("sleepyUpdates are munged (0)"));
              }
              Int i1 = 2 * (i + 1) - 1;
              Int i2 = 2 * (i + 1);
              if (i1 < sz) {
                UnsignedInt pri1 = m_sleepyUpdates[i1]->friend_getPriority();
                DEBUG_ASSERTCRASH(pri <= pri1,
                                  ("sleepyUpdates are munged (1)"));
              }
              if (i2 < sz) {
                UnsignedInt pri2 = m_sleepyUpdates[i2]->friend_getPriority();
                DEBUG_ASSERTCRASH(pri <= pri2,
                                  ("sleepyUpdates are munged (2)"));
              }
            }
#endif
          }
          ...
          void GameLogic::eraseSleepyUpdate(Int i) {
            ...
            DEBUG_ASSERTCRASH(i >= 0 && i < m_sleepyUpdates.size(),
                              ("bad sleepy idx"));
          ...
          // ------------------------------------------------------------------------------------------------
          inline Bool isLowerPriority(const UpdateModulePtr a,
                                      const UpdateModulePtr b) {
          ...
          Int GameLogic::rebalanceParentSleepyUpdate(Int i) {
            ...
            Int parent = ((i + 1) >> 1) - 1;
            while (parent >= 0 && isLowerPriority(m_sleepyUpdates[parent],
                                                  m_sleepyUpdates[i])) {
...
// version present for clarity. (Yes, this is worth doing.) (srj)
            ...
            UpdateModulePtr *pSZ =
                &m_sleepyUpdates[0] +
                m_sleepyUpdates.size(); // yes, this is off the end.

            while (pChild < pSZ) {
              // choose the higher-priority of the two children; we must be
              // higher-pri than that.
        ...
        child = ((i) << 1) + 1;
          ...
          UpdateModulePtr GameLogic::peekSleepyUpdate() const {
            ...
            DEBUG_ASSERTCRASH(u->friend_getIndexInLogic() == 0,
                              ("index mismatch: expected %d, got %d", 0,
          ...
          void GameLogic::friend_awakenUpdateModule(
              Object * obj, UpdateModulePtr u, UnsignedInt whenToWakeUp) {
            ...
            if (u == m_curUpdateModule) {
              DEBUG_CRASH(("You should not call setWakeFrame() from inside "
                           "your update(), because "
                           "it will be ignored, in favor of the return code "
                           "from update."));
              return;
            }
            ...
            if ((now > 0) && (u->friend_getNextCallFrame() == now) &&
                (whenToWakeUp == now + 1)) {
              // subtle but important case: if we already awake, and someone
              // calls setWakeFrame(self, UPDATE_SLEEP_NONE), we don't want to
              // reset our wake frame, since that would prevent us from getting
              // called THIS frame. since UPDATE_SLEEP_NONE really means "wake
              // up as soon as possible", we don't want to change our status if
              // we are already awake. (srj)
              return;
            }
            ...
            if (obj->isInList(&m_objList)) {
              if (idx < 0 || idx >= m_sleepyUpdates.size()) {
                RELEASE_CRASH(
                    "fatal error! sleepy update module illegal index.");
                return;
              }
...
#define DO_SINGLE_KIND
            ...
            static double timeAll, timeAllNoAnim, timeNoPart, timeNoSpawn,
                timeLogic, timeLogicNoAnim;
            static float drawCallAll, drawCallNoPart, drawCallNoSpawn,
                drawCallLogic;
            ...
            if (timeFrames > 0) {
              ...
              //		Real timeToUpdateMicrosec =
              //timeToUpdate*1E6/(TIME_FRAMES *
              // TOTAL_UNITS);
              ...
              if (mode == LOGIC) {
                ...
                drawCallLogic =
                    (float)drawCallTotal /
                    (float)(TIME_FRAMES *
                            TOTAL_UNITS); // 100 units for TIME_FRAMES
              ...
              sprintf(remark, "All %f: (%d ms) for %d %s's\n", timeAll,
                      (Int)(timeAll * 1000 / TIME_FRAMES), TOTAL_UNITS,
                      thingName.str());
              DEBUG_LOG((remark));
              ...
              if (g_UT_commaLog) {
                ...
                sprintf(
                    remark, "%f,%d,%f,%d,%f,%d,%f,%d,%s,%s,%s,%s,%f,%f,%f\n",
                    timeAll, (Int)(timeAll * 1000 / TIME_FRAMES), timeNoPart,
            ...
            if (g_UT_curThing && g_UT_startTiming) {
              ...
              while (g_UT_curThing->friend_getNextTemplate() &&
                     g_UT_curThing->friend_getNextTemplate()->getName() !=
                         SINGLE_UNIT)
            ...
            if (g_UT_startTiming && g_UT_curThing && !g_UT_gotUnit) {
              ...
              while (!g_UT_gotUnit) {
                ...
                if (btt->getDefaultOwningSide() != sides[side]) {
                  if (sides[side] == "*") {
                    if (btt->getDefaultOwningSide()
                            .isEmpty()) // wildcard for unspecified side
                      unspecified = TRUE;
                    else
                      continue;
                  } else
                ...
                for (Int test = 0; test < sizeof(illegalTemplateNames);
                     ++test) {
                ...
                //			if (btt->getName()
                //endsWith("EMPPulseBomb"))
                // continue; // 100 overloads system. 			if
                // (btt->getName() endsWith("GLAAngryMobRockProjectileObject"))
                // continue;
                // // 100 overloads system. 			if
                // (btt->getName() endsWith("ClusterMinesBomb")) continue; //
                // 100 overloads system. if (btt->getName()
                // endsWith("BlackNapalmFirestormSmall")) continue; // 100
                // overloads system. 			if (btt->getName()
                // endsWith("CabooseFullOfTerrorists")) continue; // 100
                // overloads system. if (btt->getName()
                // endsWith("GLAAngryMobMolotovCocktailProjectileObject"))
                // continue; // 100 overloads system. 			if
                // (btt->getName().startsWith("Firestorm")) continue;	// 100
                // crashes if (btt->getName().startsWith("Avalanche"))
                // continue;	// 100 crashes 			if
                // (btt->getName().startsWith("InfernoTankShell")) continue;
                // // 100 crashes
                //
                //			if (btt->getName()
                // endsWith("ChinaArtilleryBarrageShell") continue; // 100 takes
                // really, freaking long. Doesn't crash jba. if (btt->getName()
                // endsWith("ChinaTankOverlordBattleBunker") continue; // 100
                // seems to hang gth. 			if (btt->getName()
                // endsWith("ChinaTankOverlordPropagandaTower") continue; // 100
                // seems to hang gth. 			if (btt->getName()
                // endsWith("ChinaTankOverlordGattlingCannon") continue; // 100
                // seems to hang gth. 			if
                // (btt->getName().startsWith("CINE")) continue;
                // if (btt->getName() endsWith("GLAInfantryAngryMobNexus")
                // continue;
                //
                //      //missiondisk perps
                //
                //      if (btt->getName() == "AmericaAircraftCarrier")
                //      continue;
...
#define dont_DO_ATTACK
                ...
                attackedPos.z = TheTerrainLogic->getGroundHeight(attackedPos.x,
                                                                 attackedPos.y);
                ...
                for (i = 0; i < 10; i++) {
                  for (j = 0; j < 10; j++) {
                    ...
                    pos.z = TheTerrainLogic->getGroundHeight(pos.x, pos.y);
                    Team *team =
                        ThePlayerList->getNthPlayer(1)->getDefaultTeam();
...
#define dont_DO_TREE 1
                    ...
                    if (obj) {
                      ...
                      // Now onCreates were called at the constructor.  This
                      // magically created thing needs to be considered as Built
                      // for Game specific stuff.
                      for (BehaviorModule **m = obj->getBehaviorModules(); *m;
                           ++m) {
                        CreateModuleInterface *create = (*m)->getCreate();
                        if (!create)
                          continue;
          ...
          // ------------------------------------------------------------------------------------------------
          /** Update all objects in the world by invoking their update()
           * methods. */
          // ------------------------------------------------------------------------------------------------
          void GameLogic::update(void) {
            ...
            if (m_frame % 900 == 0) {
              DjLog("GameLogic::update - Heartbeat - Frame: %d", m_frame);
            }
            ...
            // Note - TerrainLogic update needs to happen after ScriptEngine
            // update, but before object updates.  jba. This way changes in
            // bridges are noted in the script engine before being cleared in
            // TerrainLogic->update
            {
              TheTerrainLogic->UPDATE();
            }

            // force CRC calculation, so we can keep a cache of the last N CRCs.
            // We do this right where the recorder would be getting the CRC
            // anyway, so replays can get the CRCs from the exact instant in
      ...
      Bool generateForSolo =
          isSoloGameOrReplay && ((m_frame % REPLAY_CRC_INTERVAL) == 0);
            ...
            if (generateForSolo || generateForMP) {
              ...
              GameMessage *msg =
                  newInstance(GameMessage)(GameMessage::MSG_LOGIC_CRC);
              msg->appendIntegerArgument(m_CRC);
              msg->appendBooleanArgument(isPlayback);

              // TheSuperHackers @info helmutbuhler 13/04/2025
              // During replay simulation, we bypass TheMessageStream and
              // instead put the CRC message directly into TheCommandList
              // because we don't update TheMessageStream during simulation.
              GameMessageList *messageList = TheMessageStream;
              if (TheRecorder && TheRecorder->getMode() ==
                                     RECORDERMODETYPE_SIMULATION_PLAYBACK)
...
#ifdef ALLOW_NONSLEEPY_UPDATES
            {
              for (std::list<UpdateModulePtr>::const_iterator it =
                ...
                DisabledMaskType dis =
                    u->friend_getObject()->getDisabledFlags();
                if (!dis.any() ||
...

            {
              while (!m_sleepyUpdates.empty()) {
                ...
                if (!dis.any() ||
                  ...
                  // DEBUG_LOG(("calling update %08lx (%d
                  // %d)...",update,update->friend_getNextCallFrame(),update->friend_getNextCallPhase()));
                  m_curUpdateModule = u;

                  sleepLen = u->update();
                  DEBUG_ASSERTCRASH(sleepLen > 0,
                                    ("you may not return 0 from update"));
                  if (sleepLen < 1)
            ...
            {
              // Handle disabled statii (and re-enable objects once frame
              // matches)
          ...
          void GameLogic::addObjectToLookupTable(Object * obj) {
            ...
            while (newID >=
                   m_objVector.size()) // Fail case is hella rare, so faster to
                                       // double up on size() call
          ...
          void GameLogic::removeObjectFromLookupTable(Object * obj) {
            ...
            // sanity
            // TheSuperHackers @fix Mauller/Xezon 24/04/2025 Prevent out of
            // range access to vector lookup table
            if (obj == NULL ||
                static_cast<size_t>(obj->getID()) >= m_objVector.size())
              return;
          ...
          // ------------------------------------------------------------------------------------------------
          /** Given an object, register it with the GameLogic and give it a
           * unique ID. */
          // ------------------------------------------------------------------------------------------------
        ...
        if (when == 0)
          ...
          // ------------------------------------------------------------------------------------------------
          /** create an object based on the thing template, the reason that this
           * is here and not in the ThingFactory is so that we can mirror what
           * the creation process is on the client side because clients have
           * specific device dependent drawables (such as a W3DDrawable derived
           * from Drawable). here we will allocate an object of the correct type
           * based on thing template properties
           *
           * if we want to have the thing manager actually contain the pools of
           * object and drawable storage it seems OK to have it be friends with
           * the GameLogic/Client for those purposes, or we could put the
           * allocation pools in the GameLogic and GameClient themselves */
          // ------------------------------------------------------------------------------------------------
          Object *GameLogic::friend_createObject(
              const ThingTemplate *thing,
              const ObjectStatusMaskType &statusBits, Team *team) {
            Object *obj;
          ...
          // ------------------------------------------------------------------------------------------------
          /** Mark the object as destroyed, and place on list for deletion at
           * the end of the next update. This is the only interface to destroy
           * objects - objects cannot be directly deleted. */
          // ------------------------------------------------------------------------------------------------
          void GameLogic::destroyObject(Object * obj) {
            ...
            // statemachine will try to do stopping logic, which uses virtual
            // functions and deleted modules, which will crash us.
            ...
            // add to end of destruction list, in case something is being
            // destroyed and trying to destroy subobjects
          ...
          UnsignedInt GameLogic::getCRC(Int mode, AsciiString deepCRCFileName) {
...
#ifdef DEBUG_CRC
              // TheSuperHackers @info helmutbuhler 04/09/2025
              // This allows you to save the binary data that is involved in the
              // crc calculation to a binary file per frame. This was apparently
              // used early in development and isn't that useful, because
              // diffing that binary data is very difficult. The CRC logging is
              // much easier to diff and also more granular than this because it
              // can capture changes between two frames.
            ...
            if (isInGameLogicUpdate()) {
              CRCGEN_LOG(("CRC at start of frame %d is 0x%8.8X", m_frame,
                          xferCRC->getCRC()));
            }
            ...
            if (isInGameLogicUpdate()) {
              CRCGEN_LOG(("CRC after objects for frame %d is 0x%8.8X", m_frame,
                          xferCRC->getCRC()));
            }
            ...
            if (isInGameLogicUpdate()) {
              CRCGEN_LOG(("CRC after AI for frame %d is 0x%8.8X", m_frame,
                          xferCRC->getCRC()));
            }
            ...
            if (xferCRC->getXferMode() == XFER_SAVE) {
              ...
              xferCRC->xferAsciiString(&marker);
              TheGameState->friend_xferSaveDataForCRC(
                  xferCRC, SNAPSHOT_DEEPCRC_LOGICONLY);
            }
          ...
          void GameLogic::exitGame() {
            // TheSuperHackers @fix The logic update must not be halted to
            // process the game exit message.
            setGamePaused(FALSE);
          ...
          void GameLogic::sendObjectCreated(Object * obj) {
            ...
            /// @todo COLIN ... shouldn't we have a check here for existing
            /// drawable!!!!!
          ...
          void GameLogic::sendObjectDestroyed(Object * obj) {
            // Because this implementation is a bridge between the Logic and
            // Interface, we must take extra care to handle such cases as when
            // the system it shutting down.
          ...
          void GameLogic::setGamePausedInFrame(UnsignedInt frame,
                                               Bool disableLogicTimeScale) {
            if (frame >= m_frame) {
              m_pauseFrame = frame;

              if (disableLogicTimeScale) {
                m_logicTimeScaleEnabledMemory =
          ...
          void GameLogic::setGamePaused(Bool paused, Bool pauseMusic,
            ...
            // GameUnpaused
            // Set mouse the way it "was" <--- Was counting on right answer
            // being set in Pause.
          ...
          void GameLogic::pauseGameSound(Bool paused) {
            ...
            if (paused) {
              TheAudio->pauseAudio(
                  (AudioAffect)(AudioAffect_All & ~AudioAffect_Music));

#if 0 // Kris added this code some time ago. I'm not sure why -- the pauseAudio
      // should stop the ambients by itself. Everything seems to work fine
      // without it and it's messing up my custom ambient code. Hopefully he can
      // explain it to me, but until he gets back, I'm disabling it. -Ian
		...
		while( drawable )
		{
          ...
          void GameLogic::pauseGameInput(Bool paused) {
            ...
            if (paused) {
              // remember the state of the mouse/input so we can return to the
              // same state once we "unpause"
          ...
          void GameLogic::processProgressComplete(Int playerId) {
            if (playerId < 0 || playerId >= MAX_SLOTS) {
              DEBUG_ASSERTCRASH(FALSE,
                                ("GameLogic::processProgressComplete, Invalid "
                                 "playerid was passed in %d",
                                 playerId));
              return;
            }
            if (m_progressComplete[playerId] == TRUE) {
              DEBUG_LOG(("GameLogic::processProgressComplete, playerId %d is "
                         "marked TRUE "
          ...
          void GameLogic::testTimeOut(void) {
            ...
            for (Int i = 0; i < MAX_SLOTS; ++i) {
              ...
              if (m_progressComplete[i])
                continue;
              if (m_progressCompleteTimeout[i] + PROGRESS_COMPLETE_TIMEOUT >
                  curTime)
                return;
            }
          ...
          // ------------------------------------------------------------------------------------------------
          Bool GameLogic::findBuildableStatusOverride(
              const ThingTemplate *tt, BuildableStatus &bs) const {
            if (tt) {
          ...
          // ------------------------------------------------------------------------------------------------
          void GameLogic::setControlBarOverride(
          ...
          Bool GameLogic::findControlBarOverride(
            ...
            ControlBarOverrideMap::const_iterator it =
                m_controlBarOverrides.find(buf);
...
#ifdef DUMP_PERF_STATS
          // ------------------------------------------------------------------------------------------------
          void GameLogic::getAIMetricsStatistics(
              UnsignedInt * numAI, UnsignedInt * numMoving,
              UnsignedInt * numAttacking, UnsignedInt * numWaitingForPath,
          ...
          // ------------------------------------------------------------------------------------------------
          /** Given a string name, find the object TOC entry (if any) associated
           * with it
           */
          // ------------------------------------------------------------------------------------------------
          GameLogic::ObjectTOCEntry *GameLogic::findTOCEntryByName(
              AsciiString name) {

            for (ObjectTOCListIterator it = m_objectTOC.begin();
                 it != m_objectTOC.end(); ++it)
              if ((*it).name == name)
                return &(*it);
          ...
          // ------------------------------------------------------------------------------------------------
          GameLogic::ObjectTOCEntry *GameLogic::findTOCEntryById(
              UnsignedShort id) {
          ...
          void GameLogic::xferObjectTOC(Xfer * xfer) {
            ...
            if (xfer->getXferMode() == XFER_SAVE) {
              ...
              for (Object *obj = getFirstObject(); obj;
                   obj = obj->getNextObject()) {
          ...
          void GameLogic::prepareLogicForObjectLoad(void) {
            ...
            //
            // this is a band-aid :(
            // when loading from a map file, objects were created for the
            // bridges, towers, walls etc. since we're going to load that data
            // from the save game file we will destroy any of those bridge
            // objects and tower objects so that we can create them from the
            // save data
            //
            Object *obj, *next;
            for (obj = getFirstObject(); obj; obj = next) {
              ...
              if (obj->isKindOf(KINDOF_BRIDGE)) {
                ...
                DEBUG_ASSERTCRASH(bridge,
                                  ("GameLogic::prepareLogicForObjectLoad - "
                ...
                DEBUG_ASSERTCRASH(oldObject,
                                  ("GameLogic::prepareLogicForObjectLoad - "
                                   "Unable to find old bridge object"));
                DEBUG_ASSERTCRASH(oldObject == obj,
                                  ("GameLogic::prepareLogicForObjectLoad - obj "
                                   "!= oldObject"));

                //
                // destroy the 4 towers that are attached to this old object
                // (they will be loaded from the save game file
                //
                Object *oldTower;
            ...
            // process the destruction of these objects immediately before we
            ...
            DEBUG_ASSERTCRASH(
                getFirstObject() == NULL,
                ("GameLogic::prepareLogicForObjectLoad - There are still "
                 "objects loaded "
           ...
           * 4: Added block markers around object data, no version checking is
           * done and therefore this version breaks compatibility with previous
           * versions. (CBD) 5: Added xfering the BuildAssistant's sell list. 9:
           * Added m_rankPointsToAddAtGameStart, or else on a load game, your
           * RestartGame button will forget your exp 10: xfer
          ...
          void GameLogic::xfer(Xfer * xfer) {
            ...
            //
            // note that we do not do the id counter here, we did it in the game
            // state block because it's important to do that part very early in
            // the load process
            //
            ...
            //
            // xfer a table of contents that contain thing template and
            // indentifier pairs. this table of contents is good for this save
            // file only as unique numbers are only generated and stored for the
            // actual things that are on this map
            //
            xferObjectTOC(xfer);

            // when loading, we need to clean up bridges and get them ready for
            // a load
            ...
            if (xfer->getXferMode() == XFER_SAVE) {
              ...
              for (obj = getFirstObject(); obj; obj = obj->getNextObject()) {
                ...
                if (tocEntry == NULL) {
                  ...
                  DEBUG_CRASH(
                      ("GameLogic::xfer - Object TOC entry not found for '%s'",
              ...
              for (UnsignedInt i = 0; i < objectCount; ++i) {
                ...
                if (thingTemplate == NULL) {
                  ...
                  DEBUG_CRASH((
                      "GameLogic::xfer - Unrecognized thing template name "
                      "'%s', "
                      "skipping.  ENGINEERS - Are you *sure* it's OK to be "
                      "ignoring "
                ...
                // end of block of data (not necessary in a load, but looks
                // symettrically nice)
                xfer->endBlock();

                // special case for wall pieces, need to add them to the
                // pathfinder
                if (obj->isKindOf(KINDOF_WALK_ON_TOP_OF_WALL))
                  TheAI->pathfinder()->addWallPiece(obj);
              }
            ...
            if (version >= 3) {
              ...
              //
              // since the save game loaded should have exactly the same number
              // of polygon triggers in it that we did from loading the map this
              // is just a sanity check here
              //
              if (sanityTriggerCount != triggerCount) {

                DEBUG_CRASH(
                    ("GameLogic::xfer - Polygon trigger count mismatch.  Save "
                     "file has a count of '%d', but map had '%d' triggers",
              ...
              if (xfer->getXferMode() == XFER_SAVE) {
                ...
                for (UnsignedInt i = 0; i < triggerCount; ++i) {
                  ...
                  if (poly == NULL) {
                    ...
                    DEBUG_CRASH(("GameLogic::xfer - Unable to find polygon "
                                 "trigger with id '%d'",
                                 triggerID));
                    throw SC_INVALID_DATA;
                  }
                ...
                //
                // force a recalculation of the pathfinding cause some of these
                // polygon triggers are water which can move during run-time,
                // and therefore affecting the area that objects can move on.
                // Also, map objects need recalculation. jba.
                //
                TheAI->pathfinder()->newMap();
              }
            }
            ...
            if (version >= 7) {
              if (xfer->getXferMode() == XFER_SAVE) {
                ...
                if (m_thingTemplateBuildableOverrides.empty() == false) {
                  DEBUG_CRASH(
                      ("GameLogic::xfer - m_thingTemplateBuildableOverrides "
            ...
            if (version >= 8) {
              ...
              if (xfer->getXferMode() == XFER_SAVE) {
                for (ControlBarOverrideMap::const_iterator it =
                  ...
                  AsciiString name = it->first;
                  AsciiString value = it->second ? it->second->getName()
                                                 : AsciiString::TheEmptyString;
                  xfer->xferAsciiString(&name);
                ...
                if (m_controlBarOverrides.empty() == false) {
                  DEBUG_CRASH(("GameLogic::xfer - m_controlBarOverrides should "
                               "be empty, "
                               "but is not"));
                  throw SC_INVALID_DATA;
                }
                ...
                for (;;) {
                  ...
                  if (value.isNotEmpty()) {
                    ...
                    DEBUG_ASSERTCRASH(
                        button != NULL,
                        ("Could not find button %s", value.str()));
                  }
          ...
          void GameLogic::loadPostProcess(void) {
            ...
            //
            // the act of loading objects can (theoretically) as a side effect
            // create other objects, our m_nextObjID that we maintain to give
            // objects unique ID is also continually climbing higher and higher
            // due to us allocating objects during load (even though those
            // objects have their ids overwritten with data from the file.  To
            // prevent the m_nextObjID from getting un-necessarily high we will
            // set it to the next available id from the objects that are now in
            ...
            for (std::vector<UpdateModulePtr>::iterator it =
                     m_sleepyUpdates.begin();
                 it != m_sleepyUpdates.end(); ++it) {
              (*it)->friend_setIndexInLogic(-1);
            }
      ...
      if (now == 0)
        now = 1;
            ...
            // go through all objects, examine each update module and put it on
            // the appropriate update list
            for (obj = getFirstObject(); obj; obj = obj->getNextObject()) {
          ...
          if (when == 0)
            ...
            // re-sort the priority queue all at once now that all modules are
            // on it
            remakeSleepyUpdate();
          }
