...
Bool SidesList::ParseSidesDataChunk(DataChunkInput &file, DataChunkInfo *info,
  ...
  file.registerParser("PlayerScriptsList", info->label,
...
static Bool ParseTeamsDataChunk(DataChunkInput &file, DataChunkInfo *info,
  ...
  while (!file.atEndOfChunk()) {
    ...
    if (sides->findSkirmishSideInfo(player)) {
      ...
      // DEBUG_LOG(("Couldn't add team %s, no player %s", teamName.str(),
      // player.str()));
    }
  }
  DEBUG_ASSERTCRASH(file.atEndOfChunk(), ("Unexpected data left over."));
...
void SidesList::prepareForMP_or_Skirmish(void) {
  ...
  for (i = 0; i < m_numSkirmishSides; i++) {
    ...
    if (m_skirmishSides[i].getScriptList() == NULL)
      continue;
  ...
  if (!gotScripts) {
    ...
    if (theInputStream.open(path)) {
      ...
      file.registerParser("PlayerScriptsList", AsciiString::TheEmptyString,
                          ScriptList::ParseScriptsDataChunk);
      file.registerParser("ScriptsPlayers", AsciiString::TheEmptyString,
                          ParsePlayersDataChunk);
      file.registerParser("ScriptTeams", AsciiString::TheEmptyString,
                          ParseTeamsDataChunk);
      if (!file.parse(this)) {
  ...
  for (i = 0; i < getNumSides(); i++) {
    ...
    if (ti) {
      ...
      if (ti->getDict()->getAsciiString(TheKey_teamOwner) != pname) {
        DEBUG_CRASH(("hmm, team owner mismatch (%s) (%s), this should not "
      ...
      DEBUG_LOG(("*** default team for player %s missing (should not be "
                 "possible), adding it...",
                 tname.str()));
      Dict d;
...
BuildListInfo::BuildListInfo(void)
    : m_nextBuildList(NULL), m_renderObj(NULL), m_shadowObj(NULL),
      m_isInitiallyBuilt(false), m_numRebuilds(0), m_angle(0),
      m_script(AsciiString::TheEmptyString), m_health(100), m_whiner(true),
      m_unsellable(false), m_repairable(true), m_objectID(INVALID_ID),
      m_objectTimestamp(0), m_underConstruction(false),
      m_isSupplyBuilding(false), m_desiredGatherers(0), m_currentGatherers(0),
      m_automaticallyBuild(true), m_priorityBuild(false),
...
void TeamsInfoRec::addTeam(const Dict *d) {
  ...
  DEBUG_ASSERTCRASH(
      m_numTeams < 2048,
      ("%d teams have been allocated (so far). This seems excessive.",
       m_numTeams));
