...
void dumpBattlePlanBonuses(const BattlePlanBonuses *b, AsciiString name,
  ...
  DEBUG_LOG(("dumpBattlePlanBonuses() %s:%d %s\n  Player %d(%ls) object %d(%s) "
             "armor:%g/%8.8X bombardment:%d, holdTheLine:%d, "
             "searchAndDestroy:%d sight:%g/%8.8X, valid:%s invalid:%s",
             fname.str(), line, name.str(), (p) ? p->getPlayerIndex() : -1,
             (p) ? ((Player *)p)->getPlayerDisplayName().str() : L"<No Name>",
             (o) ? o->getID() : -1,
             (o) ? o->getTemplate()->getName().str() : "<No Name>",
             b->m_armorScalar, AS_INT(b->m_armorScalar), b->m_bombardment,
             b->m_holdTheLine, b->m_searchAndDestroy, b->m_sightRangeScalar,
             AS_INT(b->m_sightRangeScalar),
             kindofMaskAsAsciiString(b->m_validKindOf).str(),
             kindofMaskAsAsciiString(b->m_invalidKindOf).str()));
}
...
Player::Player(Int playerIndex) {
  ...
  m_aiAssistEnabled = FALSE; // AI assist disabled by default
  init(NULL);
}
...
void Player::init(const PlayerTemplate *pt) {
  ...
  if (pt) {
    ...
    if (m_money.countMoney() == 0) {
      // TheSuperHackers @bugfix Now correctly deposits the money and fixes its
      // audio and academy issues. Note that copying the entire Money class
      // instead would also copy the player index inside of it.
      if (TheGameInfo) {
        m_money.deposit(TheGameInfo->getStartingCash().countMoney(), FALSE,
                        FALSE);
      } else {
        m_money.deposit(TheGlobalData->m_defaultStartingCash.countMoney(),
                        FALSE, FALSE);
      }
    }
  ...
  // AI assist disabled by default for human players
  m_aiAssistEnabled = FALSE;
}
...
void Player::update() {
  ...
  if (m_ai) {
    if (m_playerType == PLAYER_COMPUTER) {
      // AI player - always update
      m_ai->update();
    } else if (m_playerType == PLAYER_HUMAN && m_aiAssistEnabled) {
      // Human player - only update if assist is enabled
      m_ai->update();
    }
  }
  ...
  if (m_energy.getPowerSabotagedTillFrame() != 0 &&
      TheGameLogic->getFrame() > m_energy.getPowerSabotagedTillFrame()) {
    m_energy.setPowerSabotagedTillFrame(
        0); // Tells us we're no longer sabotaged for above check.
    onPowerBrownOutChange(!m_energy.hasSufficientPower());
  }

  // Update the academy stats (this only checks applicable things that require a
  // polling method)
  ...
  if (ThePlayerList->getLocalPlayer() == this) {
    ...
    // Only check and post the message once every second so we don't spam the
    // message stream to account for lag.
    if (now % LOGICFRAMES_PER_SECOND == 0) {
      if (TheGlobalData->m_clientRetaliationModeEnabled !=
          isLogicalRetaliationModeEnabled()) {
        // Post a logical message that will switch the retaliation mode on or
        // off.
...
void Player::setPlayerType(PlayerType t, Bool skirmish) {
  ...
  // Don't delete AI immediately - we might need it for assist mode
  // deleteInstance(m_ai);
  // m_ai = NULL;

  if (t == PLAYER_COMPUTER) {
    // AI player - always needs AI
    if (!m_ai) {
      if (skirmish || TheAI->getAiData()->m_forceSkirmishAI) {
        m_ai = new AICoopPlayer(this);
        m_ai->setAIDifficulty(DIFFICULTY_HARD);
      } else {
        m_ai = newInstance(AIPlayer)(this);
      }
    }
    m_aiAssistEnabled = true; // Always enabled for AI players
  } else if (t == PLAYER_HUMAN) {
    // Human player - AI assist is optional
    // Create AI for assist if in skirmish mode
    if (!m_ai && skirmish) {
      m_ai = new AICoopPlayer(this);
      m_ai->setAIDifficulty(DIFFICULTY_NORMAL);
    }
    m_aiAssistEnabled = false; // Disabled by default, user can toggle
  }
}
...
//=============================================================================
// AI Assist Control for Human Players
//=============================================================================
void Player::setAIAssistEnabled(Bool enabled) {
  if (m_playerType != PLAYER_HUMAN) {
    // Only human players can use AI assist
    return;
  }

  m_aiAssistEnabled = enabled;

  DjLog("Player %d (%ls): AI Assist %s", m_playerIndex,
        m_playerDisplayName.str(), enabled ? "ENABLED" : "DISABLED");

  // Create AI if needed
  if (enabled && !m_ai) {
    m_ai = new AICoopPlayer(this);
    m_ai->setAIDifficulty(DIFFICULTY_NORMAL);
    DjLog("Player %d: Created AICoopPlayer for assist mode", m_playerIndex);
  }
}

//=============================================================================
void Player::toggleAIAssist() { setAIAssistEnabled(!m_aiAssistEnabled); }
...
void Player::initFromDict(const Dict *d) {
  ...
  DEBUG_ASSERTCRASH(pt != NULL, ("PlayerTemplate %s not found -- this is an "
                                 "obsolete map (please open and resave in WB)",
                                 tmplname.str()));
...
Bool Player::computeSuperweaponTarget(const SpecialPowerTemplate *power,
                                      ...
                                      Real weaponRadius) {
  if (m_ai) {
    return m_ai->computeSuperweaponTarget(power, retPos, playerNdx,
                                          weaponRadius);
  }
...
//-------------------------------------------------------------------------------------------------
// PlayerObjectFindInfo is used to find a player's object. For example, we
// iterate through to find a player's command center, or a specific building
// capable of firing the specified special power.
//
...
//-------------------------------------------------------------------------------------------------
// Iterator function
// Find the first available command center that is naturally ours (not captured
// from enemy).
//-------------------------------------------------------------------------------------------------
static void doFindCommandCenter(Object *obj, void *userData) {
  ...
  if (info->obj == NULL && obj->isKindOf(KINDOF_COMMANDCENTER) &&
...
static void doFindSpecialPowerSourceObject(Object *obj, void *userData) {
  ...
  if (!obj->testStatus(OBJECT_STATUS_UNDER_CONSTRUCTION) &&
      !obj->testStatus(OBJECT_STATUS_SOLD) && !obj->isEffectivelyDead()) {
    if (info->spType == SPECIAL_INVALID && obj->hasAnySpecialPower()) {
      // We just care about find an object that has *any* shortcut capable
      // special power. Iterate through the special power modules and look for
      // one.
      SpecialPowerModuleInterface *spmInterface =
          obj->findAnyShortcutSpecialPowerModuleInterface();
      ...
      if (spmInterface && !spmInterface->isScriptOnly()) {
        ...
        if (!TheGlobalData->m_specialPowerUsesDelay)
          readyFrame = 0;
#endif
        // A disabled guy should only be considered as a last resort.  We need
        // it to be counted so that a disabled button can appear on the
        // shortcut.
        if (obj->isDisabled())
        ...
        if (readyFrame < TheGameLogic->getFrame()) {
          ...
          // This special power isn't ready, but it is going to be ready sooner
          // than any others we checked (or it's the first one we checked).
          info->obj = obj;
  ...
  if (!obj->testStatus(OBJECT_STATUS_UNDER_CONSTRUCTION) &&
    ...
    if (obj->hasSpecialPower(info->spType)) {
      ...
      if (spmInterface && !spmInterface->isScriptOnly()) {
        ...
        if (spmInterface->getSpecialPowerTemplate()->isSharedNSync() &&
            info->numReady == 1) {
        ...
        // A disabled guy should only be considered as a last resort.  We do not
        // want him counted here so that Disabled guys do not go in to the
        // number on the shortcut button.
        if (obj->isDisabled())
          readyFrame = UINT_MAX - 10;
...
//-------------------------------------------------------------------------------------------------
Object *
Player::findMostReadyShortcutSpecialPowerOfType(SpecialPowerType spType) {
  PlayerObjectFindInfo info;
  info.player = this;
  info.obj = NULL;
  info.spType = spType;
...
//-------------------------------------------------------------------------------------------------
Object *
Player::findMostReadyShortcutWeaponForThing(const ThingTemplate *thing,
...
//-------------------------------------------------------------------------------------------------
Object *Player::findMostReadyShortcutSpecialPowerForThing(
    const ThingTemplate *thing, UnsignedInt &mostReadyPercentage) {
  PlayerObjectFindInfo info;
  info.player = this;
  info.obj = NULL;
  info.thing = thing;
...
//-------------------------------------------------------------------------------------------------
Object *
Player::findAnyExistingObjectWithThingTemplate(const ThingTemplate *thing) {
  PlayerObjectFindInfo info;
  info.player = this;
  info.obj = NULL;
  info.thing = thing;
...
Bool Player::hasAnyShortcutSpecialPower() {
  ...
  info.obj = NULL;
  info.spType =
      SPECIAL_INVALID; // Invalid dictates that we don't care about the type.
  info.lowestReadyFrame = 0xffffffff;
  iterateObjects(doFindSpecialPowerSourceObject, &info);
  return info.obj;
}
...
void Player::preTeamDestroy(const Team *team) {
  ...
  // TheSuperHackers @bugfix Mauller/Xezon 03/05/2025 Clear the default team to
  // prevent dangling pointer usage
...
//=============================================================================
VeterancyLevel
Player::getProductionVeterancyLevel(AsciiString buildTemplateName) const {
...
void Player::killPlayer(void) {
  ...
  if (isLocalPlayer() && !TheGameLogic->isInShellGame()) {
    ...
    // Avoid multiplayer mismatch on surrender in team games when owning a base
    // structure that has been cleared of infantry before, by disabling this
    // bugged logic.
    //
    // Note that becomingLocalPlayer calls recalcApparentControllingPlayer on
    // all objects with a contain, which does two things:
    //   1. Reset the teams of all objects with empty contains
    //   2. Update m_hideGarrisonedStateFromNonallies
    //
    // Likely, the original intent was to reveal hidden garrisoned buildings for
    // the new observer. But this does not appear to work anyway, so this call
    // is useless.
    //
    // There is another bug: The Contain modules keep track of the object's team
    // before any units got inside. And that team does not get updated when the
    // player is killed and the units transfer to another team. That means
    // resetting the team sets it to neutral, and when this is done only for the
    // local player, then a mismatch occurs.
    //
    // When fixing disguises for observers or fixing the team bug in the Contain
    // classes, then this code can be used again.
#if 0
...
void sellBuildings(Object *obj, void *userData) {
  if (obj->isFactionStructure() || obj->isKindOf(KINDOF_COMMANDCENTER) ||
      obj->isKindOf(KINDOF_FS_POWER)) {
...
//=============================================================================
// Calculates the closest construction zone location based on a template. Gets
// plassed to aiPlayer
//=============================================================================
Bool Player::calcClosestConstructionZoneLocation(
    const ThingTemplate *constructTemplate, Coord3D *location) {
  if (m_ai) {
    return m_ai->calcClosestConstructionZoneLocation(constructTemplate,
                                                     location);
  }
...
Bool Player::addSkillPoints(Int delta) {
  ...
  Int pointCap = TheRankInfoStore->getRankInfo(levelCap)
                     ->m_skillPointsNeeded; // Cap at lowest point of cap level,
                                            // not highest.
...
Bool Player::addScience(ScienceType science) {
  ...
  // %s",TheScienceStore->getInternalNameForScience(science).str()));
...
void Player::addSciencePurchasePoints(Int delta) {
  ...
  // %d",m_sciencePurchasePoints,m_sciencePurchasePoints+delta));
...
Bool Player::attemptToPurchaseScience(ScienceType science) {
  if (!isCapableOfPurchasingScience(science)) {
    DEBUG_CRASH(("isCapableOfPurchasingScience: need other prereqs/points to "
                 "purchase, request is ignored!"));
    return false;
  }
...
Bool Player::setRankLevel(Int newLevel) {
  ...
  DEBUG_ASSERTCRASH(m_skillPoints >= m_levelDown && m_skillPoints < m_levelUp,
                    ("hmm, wrong"));
  // DEBUG_LOG(("Rank %d, Skill %d, down %d, up %d",m_rankLevel,m_skillPoints,
...
Player::getScienceAvailabilityTypeFromString(const AsciiString &name) {
...
// ------------------------------------------------------------------------------------------------
/** Count all the units of a given type that exist or are in any production
 * queues for a player */
// ------------------------------------------------------------------------------------------------
static void countExisting(Object *obj, void *userData) {
  ...
  // Compare templates
  if ((typeCountData->type &&
       typeCountData->type->isEquivalentTo(obj->getTemplate())) ||
      (typeCountData->linkKey != NAMEKEY_INVALID &&
       obj->getTemplate() != NULL &&
       typeCountData->linkKey ==
           obj->getTemplate()->getMaxSimultaneousLinkKey())) {
    typeCountData->count++;
  }
...
Bool Player::canBuildMoreOfType(const ThingTemplate *whatToBuild) const {
  ...
  if (maxSimultaneousOfType != 0) {
    ...
    typeCountData.checkProductionInterface =
        !whatToBuild->isKindOf(KINDOF_STRUCTURE);
...
//-------------------------------------------------------------------------------------------------
UnsignedInt
Player::getOrStartSpecialPowerReadyFrame(const SpecialPowerTemplate *temp) {
...
static void localApplyBattlePlanBonusesToObject(Object *obj, void *userData) {
  ...
  if (objectToValidate && objectToValidate->isAnyKindOf(bonus->m_validKindOf)) {
    ...
    if (!objectToValidate->isAnyKindOf(bonus->m_invalidKindOf)) {
      ...
      if (!isProjectile) {
        DEBUG_LOG(
            ("Is not projectile.  Armor scalar is %g", bonus->m_armorScalar));
        ...
        if (bonus->m_armorScalar != 1.0f) {
          ...
          DEBUG_LOG(
              ("After apply, armor scalar is %g", body->getDamageScalar()));
        }
...
// ------------------------------------------------------------------------------------------------
void Player::setUnitsVisionSpied(Bool setting, KindOfMaskType whichUnits,
                                 PlayerIndex byWhom) {
  VisionSpiedStruct data;
  data.setting = setting;
  data.whichUnits = whichUnits;
  data.byWhom = byWhom;
  // Being spied is now a property of the unit, not us, since we can spy only a
...
void Player::xfer(Xfer *xfer) {
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    for (UnsignedShort i = 0; i < upgradeCount; ++i) {
      ...
      if (upgradeTemplate == NULL) {
        ...
        DEBUG_CRASH(
            ("Player::xfer - Unable to find upgrade '%s'", upgradeName.str()));
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    if (!m_kindOfPercentProductionChangeList.empty()) {
      ...
      DEBUG_CRASH(("Player::xfer - m_kindOfPercentProductionChangeList should "
                   "be empty but is not"));
      throw SC_INVALID_DATA;
    }
  ...
  if (version < 4) {
    ...
    if (xfer->getXferMode() == XFER_SAVE) {
      ...
      if (!m_specialPowerReadyTimerList
               .empty()) // sanity, list must be empty right now
      {
        DEBUG_CRASH(("Player::xfer - m_specialPowerReadyTimerList should be "
                     "empty but is not"));
        throw SC_INVALID_DATA;
      }
