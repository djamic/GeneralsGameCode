...
public:
  ...
  static Bool ParseSidesDataChunk(DataChunkInput &file, DataChunkInfo *info,
                                  void *userData);
  ...
  void prepareForMP_or_Skirmish(
      void); // After a map is loaded, save & clear any players.
...
class BuildListInfo : public MemoryPoolObject, public Snapshot {
  ...
  // friend doesn't play well with MPO... it reveals delete, which shouldn't be
  // visible, even to friends (srj)
  // friend class SidesInfo;  // This is essentially a component of the
  // SidesInfo data structure. friend class SidesList;  // This is essentially a
  // component of the SidesInfo data structure. friend class BuildList;  // This
  // is the class in WB that edits build lists. friend class BuildListTool; //
  // Other class in wb that edits build lists. friend class WBDocUndoable; //
  // Other class in wb that edits build lists. friend class Player; friend class
  // AISideBuildList; friend class AISkirmishPlayer;
...
public:
  ...
  BuildListInfo(void);
  //~BuildListInfo(void);
  /////< Note that deleting the head of a list deletes all linked objects in the
  //list.

  static void parseStructure(INI *ini, void *instance, void * /*store*/,
                             const void * /*userData*/);
...
protected:
  ...
  AsciiString
      m_templateName; ///< The thing template name for this model's info.
  ...
  Bool m_isInitiallyBuilt;    ///< Whether the building is built at the start of
                              ///< the game.
  ...
  Bool m_unsellable;
  Bool m_repairable;
  Bool m_automaticallyBuild; ///< If true, the ai will build.  If false, script
                             ///< has to enable this.
  ...
  ObjectID m_resourceGatherers[MAX_RESOURCE_GATHERERS]; ///< gatherers for this
                                                        ///< supply center type
                                                        ///< building.
...
public:
  // srj sez: naughty public access to avoid 'friend' -- should be friend for
  // JUST THESE, but hey
...
public:
  ...
  void setAutomaticBuild(Bool val) { m_automaticallyBuild = val; }
...
inline Bool BuildListInfo::isBuildable(void) {
  if (getNumRebuilds() > 0 ||
      getNumRebuilds() == BuildListInfo::UNLIMITED_REBUILDS)
    return true;
