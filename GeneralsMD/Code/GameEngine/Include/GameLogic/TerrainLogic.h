...
enum WaypointID CPP_11( : Int){INVALID_WAYPOINT_ID = 0x7FFFFFFF};
...
class Waypoint : public MemoryPoolObject {
  ...
  friend class AICoopPlayer; // Allow AICoopPlayer to create waypoints for air
                             // patrol
public:
  // Placement new support for manual creation (Antigravity fix)
  void *operator new(size_t, void *ptr) { return ptr; }
  void operator delete(void *, void *) {}

  Waypoint(WaypointID id, AsciiString name, const Coord3D *pLoc,
...
public:
  ...
  Coord3D fromLeft, fromRight, toLeft,
      toRight;     /// The 4 corners of the rectangle that the bridge covers.
...
public:
  ...
  /// Get the height for an object on bridge.  Note - assumes object is on
  /// bridge. Use isPointOnBridge to check.
  ...
  Bool isCellOnEnd(
      const Region2D *cell); // Is pathfind cell on the sides of the bridge
  Bool isCellOnSide(
      const Region2D *cell); // Is pathfind cell on the end of the bridge
  Bool isCellEntryPoint(
      const Region2D *cell); // Is pathfind cell an entry point to the bridge

  void setBridgeObjectID(ObjectID id) { m_bridgeInfo.bridgeObjectID = id; }
  void setTowerObjectID(ObjectID id, BridgeTowerType which) {
    m_bridgeInfo.towerObjectID[which] = id;
  }
};
...
public:
  ...
  virtual Real getLayerHeight(Real x, Real y, PathfindLayerEnum layer,
                              Coord3D *normal = NULL, Bool clip = true) const;
  virtual void getExtent(Region3D *extent) const {
    DEBUG_CRASH(("not implemented"));
  } ///< @todo This should not be a stub - this should own this functionality
  ...
  virtual Bool isClearLineOfSight(const Coord3D &pos,
                                  const Coord3D &posOther) const;

  virtual AsciiString getSourceFilename(void) { return m_filenameString; }

  virtual PathfindLayerEnum alignOnTerrain(Real angle, const Coord3D &pos,
                                           Bool stickToGround, Matrix3D &mtx);

  virtual Bool isUnderwater(Real x, Real y, Real *waterZ = NULL,
                            Real *terrainZ = NULL); ///< is point under water
  virtual Bool isCliffCell(Real x, Real y) const;   ///< is point cliff cell
  virtual const WaterHandle *
  getWaterHandle(Real x, Real y); ///< get water handle at this location
  virtual const WaterHandle *
  getWaterHandleByName(AsciiString name); ///< get water handle by name
  virtual Real
  getWaterHeight(const WaterHandle *water); ///< get height of water table
  virtual void
  setWaterHeight(const WaterHandle *water, Real height, Real damageAmount,
                 Bool forcePathfindUpdate); ///< set height of water table
  virtual void changeWaterHeightOverTime(
      const WaterHandle *water, Real finalHeight, Real transitionTimeInSeconds,
  ...
  /// Return true if the waypoint path containint pWay is labeled with the
  /// label.
  ...
  /// Find the bridge at a location.  NULL means no bridge. Note that the layer
  /// value will be used to resolve crossing bridges.
  virtual Bridge *findBridgeLayerAt(const Coord3D *pLoc,
                                    PathfindLayerEnum layer,
                                    Bool clip = true) const;

  ///  Returns true if the object is close enough to interact with the bridge
  ///  for pathfinding.
  virtual Bool
  objectInteractsWithBridgeLayer(Object *obj, Int layer,
                                 Bool considerBridgeHealth = true) const;

  ///  Returns true if the object is close to one or the other end of the
  ///  bridge.
  virtual Bool objectInteractsWithBridgeEnd(Object *obj, Int layer) const;

  virtual Drawable *pickBridge(const Vector3 &from, const Vector3 &to,
                               Vector3 *pos);

  virtual void addBridgeToLogic(
      BridgeInfo *pInfo, Dict *props,
      AsciiString bridgeTemplateName); ///< Adds a bridge's logical info.
  virtual void addLandmarkBridgeToLogic(
      Object *bridgeObj);                    ///< Adds a bridge's logical info.
  virtual void deleteBridge(Bridge *bridge); ///< remove a bridge

  virtual void
  updateBridgeDamageStates(void); ///< Updates bridge's damage info.

  Bool anyBridgesDamageStatesChanged(void) {
    return m_bridgeDamageStatesChanged;
  ...
  void
  getBridgeAttackPoints(const Object *bridge,
                        TBridgeAttackInfo *info); ///< Get bridge attack points.

  PathfindLayerEnum getLayerForDestination(const Coord3D *pos);

  // this is just like getLayerForDestination, but always return the highest
  // layer that will be <= z at that point (unlike getLayerForDestination, which
  // will return the closest layer)
  PathfindLayerEnum
  getHighestLayerForDestination(const Coord3D *pos,
  ...
  void
  createCraterInTerrain(Object *obj); ///< Flatten the terrain under a building.
...
protected:
  ...
  static Bool parseWaypointDataChunk(DataChunkInput &file, DataChunkInfo *info,
                                     void *userData);
  /// Chunk parser callback.
  Bool parseWaypointData(DataChunkInput &file, DataChunkInfo *info,
                         void *userData);
  ...
  void findAxisAlignedBoundingRect(const WaterHandle *waterHandle,
                                   Region3D *region);
  ...
  static WaterHandle m_gridWaterHandle; ///< water handle for the grid water (we
                                        ///< only presently have one)

  //
  // we will force a limit of MAX_DYNAMIC_WATER as the max dynamically changable
  // water tables for a map.  We could use a list, but eh, this is fine and
  // small anyway
  //
  enum { MAX_DYNAMIC_WATER = 64 };
  struct DynamicWaterEntry {
    const WaterHandle *waterTable; ///< handle to water table to edit
    Real changePerFrame; ///< how much height to add to the water each frame
                         ///< (negative=lowering)
    Real targetHeight;   ///< the target height we want to be at
    Real
        damageAmount; ///< amount of damage to do to objects that are underwater
    Real currentHeight; ///< we need to keep track of this ourselves cause some
                        ///< water height are represented with ints
...
extern void makeAlignToNormalMatrix(Real angle, const Coord3D &pos,
