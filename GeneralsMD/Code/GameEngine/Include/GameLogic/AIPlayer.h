 ...
 */
class WorkOrder : public MemoryPoolObject, public Snapshot {

  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(WorkOrder, "WorkOrder")

public:
  WorkOrder()
      : m_thing(NULL), m_factoryID(INVALID_ID), m_isResourceGatherer(false),
        m_numCompleted(0), m_numRequired(1), m_next(NULL) {};

  Bool
  isWaitingToBuild(void); ///< return true if nothing is yet building this unit
  void validateFactory(Player *thisPlayer); ///< verify factoryID still refers
                                            ///< to an active object

public:
  const ThingTemplate *m_thing; ///< thing to build
  ObjectID
      m_factoryID; ///< ID of object that is building this, or zero if no-one is
  WorkOrder *m_next;
...
public:
  TeamInQueue()
      : m_workOrders(NULL), m_team(NULL), m_nextTeamInQueue(NULL),
        m_sentToStartLocation(false), m_reinforcement(false),
        m_stopQueueing(false), m_reinforcementID(INVALID_ID), m_frameStarted(0),
        m_priorityBuild(FALSE) {}

  Bool isAllBuilt(void); ///< Returns true if the team is finished building.
  Bool isBuildTimeExpired(
      void); ///< Returns true if the team has run out of build time.
  Bool isMinimumBuilt(void); ///< Returns true if the team has started building
                             ///< at least the minimum number of units.
  Bool
  includesADozer(void); ///< Returns true if the team includes a dozer unit.
  Bool areBuildsComplete(
      void); ///< Returns true if all units in factories have finished building.
  void
  disband(void); ///< Disbands the team (moves units into the default team).
...
public:
  ...
  Bool m_stopQueueing;  ///< True if we are to quit queueing units (usually
                        ///< because we ran out of build time.)
  Bool m_reinforcement; ///< True if it is a unit to reinforce an existing team.
  ObjectID m_reinforcementID; ///< True if it is a unit to reinforce an existing
                              ///< team.
};
...
public: // AIPlayer interface, may be overridden by AISkirmishPlayer.  jba.
  ...
  virtual Player *getAiEnemy(void) {
    return NULL;
  } ///< Solo AI attacks based on scripting.  Only skirmish auto-acquires an
    ///< enemy at this point.  jba.
...
public:
  ...
  void buildBySupplies(
      Int minimumCash,
      const AsciiString &thingName); ///< Builds a building by supplies.
...
protected:
  ...
  MAKE_DLINK_HEAD(
      TeamInQueue,
      TeamReadyQueue); ///< List of teams built, waiting to reach rally point.

protected:
  Bool isPossibleToBuildTeam(
      TeamPrototype *proto, Bool requireIdleFactory,
      Bool &needMoney); ///< return true if team can be considered for building
  ...
  Object *
  findFactory(const ThingTemplate *thing,
              Bool busyOK); ///< Find a factory to build a unit.  If force is
                            ///< true, may return a busy factory.
  void queueUnits(void); ///< Check the team build list, & queue up units at any
                         ///< idle factories.
...
protected:
  ...
  Int m_structureTimer; ///< Counts out the time between structures, as
                        ///< specified by ini.
  Int m_teamSeconds;    ///< How many seconds to delay between teams.

  Int m_buildDelay; ///< Delay for building in case we are resource or prereq.
                    ///< limited.
  Int m_teamDelay;  ///< Delay for teams in case we are resource or factory
                    ///< prereq. limited.
