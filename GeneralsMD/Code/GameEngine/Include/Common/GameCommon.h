...
//
// Desc:			 This is a catchall header for some basic types
// and definitions
//							needed by various bits
//of the GameLogic/GameClient, but that 							we haven't found a good place for yet.
//Hopefully this file 							should go away someday, but for now is a convenient spot.
//
//-----------------------------------------------------------------------------

#pragma once

#define DONT_ALLOW_DEBUG_CHEATS_IN_RELEASE ///< Take of the DONT to get cheats
                                           ///< back in to release
...
// ----------------------------------------------------------------------------------------------
enum {
  BaseFps = 30, // The historic base frame rate for this game. This value must
                // never change.
  LOGICFRAMES_PER_SECOND = WWSyncPerSecond,
  MSEC_PER_SECOND = 1000
};
const Real LOGICFRAMES_PER_MSEC_REAL =
    (((Real)LOGICFRAMES_PER_SECOND) / ((Real)MSEC_PER_SECOND));
const Real MSEC_PER_LOGICFRAME_REAL =
    (((Real)MSEC_PER_SECOND) / ((Real)LOGICFRAMES_PER_SECOND));
...
// call ceil() on the result, so that partial frames get converted to full
// frames!
...
inline Real ConvertAccelerationInSecsToFrames(Real distPerSec2) {
  // this looks wrong, but is the correct conversion factor.
  const Real SEC_PER_LOGICFRAME_SQR =
      (SECONDS_PER_LOGICFRAME_REAL * SECONDS_PER_LOGICFRAME_REAL);
  return (distPerSec2 * SEC_PER_LOGICFRAME_SQR);
}

// ----------------------------------------------------------------------------------------------
inline Real
ConvertAngularVelocityInDegreesPerSecToRadsPerFrame(Real degPerSec) {
...
enum {
  MAX_GLOBAL_GENERAL_TYPES =
      9, ///< number of playable General Types, not including the boss)
...
//-------------------------------------------------------------------------------------------------
enum GameDifficulty CPP_11( : Int){DIFFICULTY_EASY, DIFFICULTY_NORMAL,
                                   DIFFICULTY_HARD,
...
//-------------------------------------------------------------------------------------------------
/// Since an object can take up more than a single PartitionCell, this is a
/// status that applies to the whole Object
enum ObjectShroudStatus CPP_11( : Int){
    OBJECTSHROUD_INVALID, ///< indeterminate state, will recompute
    OBJECTSHROUD_CLEAR,   ///< object is not shrouded at all (ie, completely
                          ///< visible)
    OBJECTSHROUD_PARTIAL_CLEAR, ///< object is partly clear (rest is shroud or
                                ///< fog)
    OBJECTSHROUD_FOGGED,        ///< object is completely fogged
    OBJECTSHROUD_SHROUDED,      ///< object is completely shrouded
    OBJECTSHROUD_INVALID_BUT_PREVIOUS_VALID, ///< indeterminate state, will
                                             ///< recompute, BUT previous status
                                             ///< is valid, don't reset (used
                                             ///< for save/load)
};
...
enum {
  NEVER = 0,
  FOREVER = 0x3fffffff // (we use 0x3fffffff so that we can add offsets and not
                       // overflow...
...
/// Veterancy level define needed by several files that don't need the full
/// Experience code.
// NOTE NOTE NOTE: Keep TheVeterencyNames in sync with these.
enum VeterancyLevel CPP_11( : Int){LEVEL_REGULAR,   LEVEL_VETERAN,
                                   LEVEL_ELITE,     LEVEL_HEROIC,

                                   LEVEL_COUNT,     LEVEL_INVALID,

                                   LEVEL_FIRST = 0, LEVEL_LAST = LEVEL_HEROIC};
...
enum CommandSourceType CPP_11( : Int){
    ...
    CMD_FROM_SCRIPT,
    CMD_FROM_AI,
    CMD_FROM_DOZER, // Special rare command when the dozer originates a command
                    // to attack a mine. Mines are not ai-attackable, and it
                    // seems deceitful for the dozer to generate a player or
                    // script command. jba.
    CMD_DEFAULT_SWITCH_WEAPON, // Special case: A weapon that can be chosen --
                               // this is the default case (machine gun vs
                               // flashbang).
...
enum AbleToAttackType CPP_11( : Int){
    _ATTACK_FORCED = 0x01, _ATTACK_CONTINUED = 0x02,
    ...
    */
    ...
    /**
            ...
            (The only current difference between this and ATTACK_NEW_TARGET is
       that disguised units are force-attackable even when stealthed.)
    */
    ...
    /**
            ...
            (The only current difference between this and ATTACK_NEW_TARGET is
       you are allowed to follow immobile shrouded units into the fog)
    */
    ...
    */
    ...
    /**
            Special case that bypasses some of the checks for units guarding
       from within tunnel networks! For example, a unit inside couldn't normally
       see outside and would fail.
    */
...
inline Bool getVeterancyLevelFlag(VeterancyLevelFlags flags,
                                  VeterancyLevel dt) {
  return (flags & (1UL << (dt - 1))) != 0;
}

inline VeterancyLevelFlags setVeterancyLevelFlag(VeterancyLevelFlags flags,
                                                 VeterancyLevel dt) {
  return (flags | (1UL << (dt - 1)));
}

inline VeterancyLevelFlags clearVeterancyLevelFlag(VeterancyLevelFlags flags,
                                                   VeterancyLevel dt) {
  return (flags & ~(1UL << (dt - 1)));
}
...
public:                                                                        \
  inline DLINK_ITERATOR<OBJCLASS> iterate_##LISTNAME() const {                 \
    DEBUG_ASSERTCRASH(!BOGUSPTR(m_dlinkhead_##LISTNAME.m_head),                \
                      ("bogus head ptr"));                                     \
    return DLINK_ITERATOR<OBJCLASS>(m_dlinkhead_##LISTNAME.m_head,             \
                                    &OBJCLASS::dlink_next_##LISTNAME);         \
  }                                                                            \
...
private:                                                                       \
  ...
  public:                                                                      \
    ...
    inline ~DLINKHEAD_##LISTNAME() {                                           \
      DEBUG_ASSERTCRASH(!m_head,                                               \
                        ("destroying dlinkhead still in a list " #LISTNAME));  \
    }                                                                          \
  };                                                                           \
...
public:                                                                        \
  ...
  Bool dlink_isInList_##LISTNAME(OBJCLASS *const *pListHead) const {           \
    DEBUG_ASSERTCRASH(!BOGUSPTR(*pListHead) &&                                 \
                          !BOGUSPTR(m_dlink_##LISTNAME.m_next) &&              \
                          !BOGUSPTR(m_dlink_##LISTNAME.m_prev),                \
                      ("bogus ptrs"));                                         \
    return *pListHead == this || m_dlink_##LISTNAME.m_prev ||                  \
           m_dlink_##LISTNAME.m_next;                                          \
  }                                                                            \
  void dlink_prependTo_##LISTNAME(OBJCLASS **pListHead) {                      \
    DEBUG_ASSERTCRASH(!dlink_isInList_##LISTNAME(pListHead),                   \
                      ("already in list " #LISTNAME));                         \
    DEBUG_ASSERTCRASH(!BOGUSPTR(*pListHead) &&                                 \
                          !BOGUSPTR(m_dlink_##LISTNAME.m_next) &&              \
                          !BOGUSPTR(m_dlink_##LISTNAME.m_prev),                \
                      ("bogus ptrs"));                                         \
  ...
  void dlink_removeFrom_##LISTNAME(OBJCLASS **pListHead) {                     \
    ...
    if (m_dlink_##LISTNAME.m_next)                                             \
      m_dlink_##LISTNAME.m_next->m_dlink_##LISTNAME.m_prev =                   \
          m_dlink_##LISTNAME.m_prev;                                           \
    if (m_dlink_##LISTNAME.m_prev)                                             \
      m_dlink_##LISTNAME.m_prev->m_dlink_##LISTNAME.m_next =                   \
          m_dlink_##LISTNAME.m_next;                                           \
    else                                                                       \
...
private:                                                                       \
  ...
  public:                                                                      \
    ...
    inline ~DLINK_##LISTNAME() {                                               \
      DEBUG_ASSERTCRASH(!m_prev && !m_next,                                    \
                        ("destroying dlink still in a list " #LISTNAME));      \
    }                                                                          \
  };                                                                           \
  DLINK_##LISTNAME m_dlink_##LISTNAME;

// ------------------------------------------------------------------------
// this is the weird C++ syntax for "call pointer-to-member-function"... see C++
// FAQ LITE for details.
...
private:
  OBJCLASS *m_cur;
  GetNextFunc m_getNextFunc; // this is the weird C++ syntax for
...
enum WhichTurretType CPP_11( : Int){TURRET_INVALID = -1,
                                    ...
                                    TURRET_MAIN = 0, TURRET_ALT,

                                    MAX_TURRETS};

// ------------------------------------------------------------------------
// this normalizes an angle to the range -PI...PI.
// TheSuperHackers @todo DO NOT USE THIS FUNCTION! Use WWMath::Normalize_Angle
// instead. Delete this.
extern Real normalizeAngle(Real angle);

// ------------------------------------------------------------------------
// this returns the difference between a1 and a2, normalized.
inline Real stdAngleDiff(Real a1, Real a2) { return normalizeAngle(a1 - a2); }

// ------------------------------------------------------------------------
// NOTE NOTE NOTE: Keep TheRelationShipNames in sync with this enum
enum Relationship CPP_11( : Int){ENEMIES, NEUTRAL, ALLIES,
