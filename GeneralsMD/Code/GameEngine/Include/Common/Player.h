...
static const char *const ScienceAvailabilityNames[] = {"Available", "Disabled",
                                                       "Hidden", NULL};
static_assert(ARRAY_SIZE(ScienceAvailabilityNames) ==
                  SCIENCE_AVAILABILITY_COUNT + 1,
...
class KindOfPercentProductionChange : public MemoryPoolObject {
  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(KindOfPercentProductionChange,
...
struct SpecialPowerReadyTimerType {
  SpecialPowerReadyTimerType() { clear(); }
  void clear(void) {
    m_readyFrame = 0xffffffff;
    m_templateID = INVALID_ID;
  };

  UnsignedInt m_templateID; ///< The ID of the specialpower template associated
                            ///< with this timer... no dupes allowed
  UnsignedInt
      m_readyFrame; ///< on what game frame will this special power be ready?
};

// ------------------------------------------------------------------------------------------------
typedef std::hash_map<PlayerIndex, Relationship, std::hash<PlayerIndex>,
                      std::equal_to<PlayerIndex>>
    PlayerRelationMapType;
class PlayerRelationMap : public MemoryPoolObject, public Snapshot {

  MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE(PlayerRelationMap,
                                          "PlayerRelationMapPool")

public:
  PlayerRelationMap(void);
  // virtual destructor provided by memory pool object

  /** @todo I'm jsut wrappign this up in a nice snapshot object, we really
   * should isolate m_map from public access and make access methods for our
   * operations */
...
// ----------------------------------------------------------------------------------------------
/**
        A "Player" as an entity that contains the persistent info of the Player,
   as well as containing transient mission data.

        Some of the Player's attributes persist between missions, whereas others
   are "transient" and only have meaning in a mission, wherein they change a lot
   (current tech tree state, current buildings built, units trained, money,
   etc). (For clarity of nomenclature, we'll refer to all "persistent" vs
   "non-persistent" attributes, since "transient" has an unclear duration.)

        A "Player" consists of an entity controlling a single set of units in a
   mission. A Player may be human or computer controlled.

        All Players have a (transient) "Player Index" associated which allows us
   to do some shorthand for representing Players in some cases (mainly in
   bitfields). This is always a 0-based number (-1 is used as an illegal index).
   Note that a logical conclusion of this is that it makes it desirable to limit
   the maximum number of simultaneous Players to 32, but that seems quite
   reasonable.

        All persistent data associated with the Player will be saved to local
   storage and/or synchronized with one of our servers for online play.

        Note, simple accessor methods are omitted for brevity... for now, you
   may infer them from the data structures.

        Note also that the class (and the relevant subclasses) are almost
   certainly incomplete; a short perusal of the RA2 equivalent class(es) have
   waaay more tweaky data fields, some of which will probably be necessary (in
   some form, at some point), some of which won't ever be necessary. Rather than
   trying to guess at all of them here, we're limiting the initial setup to be
   the almost-certainly-necessary, filling in the extra things as needed. (Note
   that most of these will be non-persistent!)
*/
...
public:
  ...
  const AsciiString &getName() const { return m_playerName; }
  ...
  UnsignedInt
  getSupplyBoxValue(); ///< Many things can affect the alue of a crate, but at
                       ///< heart it is a GlobalData ratio.

  Energy *getEnergy() { return &m_energy; }
  const Energy *getEnergy() const { return &m_energy; }

  // adds a power bonus to this player because of energy upgrade at his power
  // plants
  ...
  ResourceGatheringManager *getResourceGatheringManager() {
    return m_resourceGatheringManager;
  }
  ...
  /// a convenience function to test the ThingTemplate against the players
  /// canBuild flags called by canBuild
  ...
  // Finds a short-cut firing special power of specified type returning the
  // first ready power or the most ready if none ready.
  Object *findMostReadyShortcutSpecialPowerOfType(SpecialPowerType spType);

  // Find specified thing template's most ready weapon.
  Object *findMostReadyShortcutWeaponForThing(const ThingTemplate *thing,
                                              UnsignedInt &mostReadyPercentage);
  Object *
  findMostReadyShortcutSpecialPowerForThing(const ThingTemplate *thing,
                                            UnsignedInt &mostReadyPercentage);

  // Finds a short-cut firing special power of any type arbitrarily.
  Bool hasAnyShortcutSpecialPower();

  // Counts available shortcut special power of specified type that can fire
  // now.
  Int countReadyShortcutSpecialPowersOfType(SpecialPowerType spType);

  /// return t if the player has the given science, either intrinsically, via
  /// specialization, or via capture.
  Bool hasScience(ScienceType t) const;
  Bool isScienceDisabled(ScienceType t)
      const; ///< Can't purchase this science because of script reasons.
  Bool isScienceHidden(ScienceType t)
      const; ///< This science is effectively hidden due to  script reasons.
  ...
  ScienceAvailabilityType
  getScienceAvailabilityTypeFromString(const AsciiString &name);

  /// return t iff the player has all sciences that are prereqs for knowing the
  /// given science
  Bool hasPrereqsForScience(ScienceType t) const;

  Bool hasUpgradeComplete(const UpgradeTemplate *upgradeTemplate)
      const; ///< does player have totally done and produced upgrade
  ...
  Bool hasUpgradeInProduction(
      const UpgradeTemplate
          *upgradeTemplate); ///< does player have this upgrade in progress
                             ///< right now
  Upgrade *addUpgrade(const UpgradeTemplate *upgradeTemplate,
                      UpgradeStatusType status); ///< add upgrade, or update
                                                 ///< existing upgrade status
  void removeUpgrade(
      const UpgradeTemplate *upgradeTemplate); ///< remove thsi upgrade from us

  /** find upgrade, NOTE, the upgrade may *NOT* be "complete" and therefore
    "active", it could be in production This function is for actually retrieving
    the Upgrade.  To test existance, use the fast bit testers hasUpgradeX()
  */
  Upgrade *findUpgrade(const UpgradeTemplate *upgradeTemplate);

  void onUpgradeCompleted(
      const UpgradeTemplate
          *upgradeTemplate); ///< An upgrade just finished, do things like tell
                             ///< all objects to recheck UpgradeModules
  ...
  ///< Power just changed at all.  Didn't make two functions so you can't forget
  ///< to undo something you didin one of them.
  ///< @todo Can't do edge trigger until after demo; make things check for power
  ///< on creation
  void onPowerBrownOutChange(Bool brownOut);

  ///< one of my command centers just fired a special power, let us reset timers
  ...
  UnsignedInt
  getOrStartSpecialPowerReadyFrame(const SpecialPowerTemplate *temp);
  void expressSpecialPowerReadyFrame(const SpecialPowerTemplate *temp,
                                     UnsignedInt frame);
  void addNewSharedSpecialPowerTimer(const SpecialPowerTemplate *temp,
                                     UnsignedInt frame);
  ...
  void disableRadar(); ///< No matter how many radar producers I have, I do not
                       ///< have radar
  void enableRadar();  ///< remove the restriction imposed by disableRadar
  Bool hasRadar() const; ///< A positive number of radar producers, plus my
                         ///< radar is not disabled
  Bool okToPlayRadarEdgeSound(); ///< just like it "sounds"
  // Battle plans effect the players abilities... so may as well add it here.
  // Also it's possible for multiple strategy centers to have the same plan, so
  // we need to keep track of that like radar. Keep in mind multiple strategy
  // centers with same plan do not stack, but different strategy centers with
  // different plans do.
  void changeBattlePlan(BattlePlanStatus plan, Int delta,
                        BattlePlanBonuses *bonus);
  Int getNumBattlePlansActive() const {
    return m_bombardBattlePlans + m_holdTheLineBattlePlans +
           m_searchAndDestroyBattlePlans;
  }
  Int getBattlePlansActiveSpecific(BattlePlanStatus plan) const;
  void applyBattlePlanBonusesForObject(
      Object *obj) const; // New object or converted object gaining our current
                          // battle plan bonuses.
  void removeBattlePlanBonusesForObject(Object *obj) const; // Object left team
  void applyBattlePlanBonusesForPlayerObjects(
      const BattlePlanBonuses *bonus); // Battle plan bonuses changing, so apply
   ...
   */
  VeterancyLevel
  getProductionVeterancyLevel(AsciiString buildTemplateName) const;

  // Friend function for the script engine's usage.
  void friend_setSkillset(Int skillSet);

  /**
          the given object has just become (or just ceased to be) a member of
     one of our teams (or subteams)
  */
  ...
  */
  ...
  /// a structure that was created has been pre-emptively removed and shouldn't
  /// be counted in the score.
  void onStructureUndone(Object *structure);

  /**
          a convenience routine to count the number of owned objects that match
     a set of ThingTemplates. You input the count and an array of
     ThingTemplate*, and provide an array of Int of the same size. It fills in
     the array to the correct counts. This is handy because we must traverse the
     player's list-of-objects only once.
  */
  void countObjectsByThingTemplate(Int numTmplates,
  ...
  */
  ...
  /**
          simply returns the number of objects owned by this player with a
     specific KindOfMaskType
  */
  Int countObjects(KindOfMaskType setMask, KindOfMaskType clearMask);

  /// Returns the closest of a given type to the given object
  Object *findClosestByKindOf(Object *queryObject, KindOfMaskType setMask,
  ...
  */
  ...
  /**
          a convenience routine to quickly check if any buildings with a
     specific KindOfType flag are owned.
  */
  ...
  */
  ...
  /**
          a convenience routine to quickly check if any buildfacilities are
     owned.
  */
  ...
  */
  Team *getDefaultTeam() {
    DEBUG_ASSERTCRASH(m_defaultTeam != NULL, ("default team is null"));
  ...
  void addToBuildList(
      Object *obj); ///< Adds this to the build list.	 Used for factories
                    ///< placed instead of in build list.
  void addToPriorityBuildList(
      AsciiString templateName, Coord3D *pos,
      Real angle); ///< Adds this to the build list.	 Used for factories
                   ///< placed instead of in build list.

  /// get the relationship between this->that.
  Relationship getRelationship(const Team *that) const;

  /// set the relationship between this->that. (note that this doesn't affect
  ...
  void setUnitsVisionSpied(Bool setting, KindOfMaskType whichUnits,
                           PlayerIndex byWhom);
  ...
  /// Build a building near a supply dump with at least cash.  Gets passed to
  /// aiPlayer.
  void buildBySupplies(Int minimumCash, const AsciiString &thingName);

  /// Build an instance of a specific building nearest specified team.  Gets
  /// passed to aiPlayer.
  void buildSpecificBuildingNearestTeam(const AsciiString &thingName,
                                        const Team *team);
  ...
  /// Gets plassed to aiPlayer
  Bool
  calcClosestConstructionZoneLocation(const ThingTemplate *constructTemplate,
                                      Coord3D *location);
  ...
  Bool
  isPlayerObserver(void) const;  // Favor !isPlayerActive() - this is used for
                                 // Observer GUI mostly, not in-game stuff
  Bool isPlayerDead(void) const; // Favor !isPlayerActive() - this is used so
                                 // OCLs don't give us stuff after death.
  Bool
  isPlayerActive(void) const; // Player is alive and not observer.
                              // !isPlayerActive() is synonymous with observing.
  ...
  // return the hotkey squad that a unit is in, or NO_HOTKEY_SQUAD if it isn't
  // in one.
  ...
  UnsignedInt getAttackedFrame(void) {
    return m_attackedFrame;
  } // Return last frame attacked.
  ...
  // Set via logical message. Options menu sets the client value in global data.
  // Player::update() detects a change, and posts a message. When the message
  // gets processed, this value gets set.
  Bool isLogicalRetaliationModeEnabled() const {
    return m_logicalRetaliationModeEnabled;
  }
  void setLogicalRetaliationModeEnabled(Bool set) {
    m_logicalRetaliationModeEnabled = set;
  }

  // AI Assist for Human Players (Hybrid Control)
  Bool isAIAssistEnabled() const { return m_aiAssistEnabled; }
...
public:
  ...
  /// reset the sciences to just the intrinsic ones from the playertemplate, if
  /// any.
  void resetSciences();
  ...
  */
  ...
  /**
          ...
          but still restrict you to purchasable sciences (ie, intrinsic and rank
     sciences are not allowed) return true if successful.
  */
...
private:
  const PlayerTemplate
      *m_playerTemplate; ///< Pointer back to the Player Template

  UnicodeString m_playerDisplayName; ///< This player's persistent name.
  Handicap m_handicap; ///< adjustment to varied capabilities (@todo: is this
                       ///< persistent or recalced each time?)
  AsciiString
      m_playerName; ///< player's itnernal name 9for matching map objects)
  NameKeyType m_playerNameKey; ///< This player's internal name (for matching
                               ///< map objects)
  ...
  Int m_radarCount; ///< # of facilities that have a radar under the players
                    ///< control
  Int m_disableProofRadarCount; ///< # of disable proof radars.  A disable proof
                                ///< one will be in both refcounts
  Bool m_radarDisabled; ///< The radar is disabled regardless of the number of
                        ///< radar objects
  Int m_bombardBattlePlans;     ///< Number of strategy centers with active
                                ///< bombardment plan
  Int m_holdTheLineBattlePlans; ///< Number of strategy centers with active hold
                                ///< the line plan
  Int m_searchAndDestroyBattlePlans; ///< Number of strategy centers with active
                                     ///< search and destroy plan
  BattlePlanBonuses *m_battlePlanBonuses;
  UpgradeMaskType
      m_upgradesInProgress; ///< Bit field of in Production status upgrades
  UpgradeMaskType m_upgradesCompleted; ///< Bit field of upgrades completed.
                                       ///< Bits are assigned by UpgradeCenter
  Energy m_energy; ///< current energy production & consumption
  MissionStats
      m_stats; ///< stats about the current mission (units destroyed, etc)
  BuildListInfo
      *m_pBuildList;  ///< linked list of buildings for PLAYER_COMPUTER.
  Color m_color;      ///< color for our units
  Color m_nightColor; ///< tweaked version of regular color to make it easier to
                      ///< see on night maps.
  ProductionChangeMap
      m_productionCostChanges; ///< Map to keep track of Faction specific
                               ///< discounts or penalties on prices of units
  ProductionChangeMap
      m_productionTimeChanges; ///< Map to keep track of Faction specific
                               ///< discounts or penalties on build times of
                               ///< units
  ProductionVeterancyMap
      m_productionVeterancyLevels; ///< Map to keep track of starting level of
                                   ///< produced units
  ...
  ResourceGatheringManager
      *m_resourceGatheringManager; ///< Keeps track of all Supply Centers and
                                   ///< Warehouses
  TunnelTracker *m_tunnelSystem; ///< All TunnelContain buildings use this part
                                 ///< of me for actual conatinment
  Team *m_defaultTeam;           ///< our "default" team.

  ScienceVec m_sciences; ///< (SAVE) sciences that we know (either intrinsically
                         ///< or via later purchases)
  ScienceVec
      m_sciencesDisabled; ///< (SAVE) sciences that we are not permitted to
                          ///< purchase "yet". Controlled by mission scripts.
  ScienceVec m_sciencesHidden; ///< (SAVE) sciences that aren't shown.
                               ///< Controlled by mission scripts.

  Int m_rankLevel;             ///< (SAVE) our RankLevel, 1...n
  Int m_skillPoints;           ///< (SAVE) our cumulative SkillPoint total
  Int m_sciencePurchasePoints; ///< (SAVE) our unspent SciencePurchasePoint
                               ///< total
  Int m_levelUp, m_levelDown;  ///< (NO-SAVE) skill points to go up/down a level
                               ///< (runtime only)
  UnicodeString m_generalName; ///< (SAVE) This is the name of the general the
                               ///< player is allowed to change.

  PlayerTeamList
      m_playerTeamPrototypes; ///< ALL the teams we control, via prototype
  PlayerRelationMap *m_playerRelations; ///< allies & enemies
  TeamRelationMap *m_teamRelations;     ///< allies & enemies

  AcademyStats
      m_academyStats; ///< Keeps track of various statistics in order to provide
                      ///< advice to the player about how to improve playing.

  Bool
      m_canBuildUnits; ///< whether the current player is allowed to build units
  Bool m_canBuildBase; ///< whether the current player is allowed to build Base
                       ///< buildings
  Bool m_observer;
  Bool m_isPreorder;
  Real m_skillPointsModifier; ///< Multiplied by skill points before they are
                              ///< applied

  Bool m_listInScoreScreen; ///< should this player be listed in the score
                            ///< screen or not.
  Bool m_unitsShouldHunt;

  Bool m_attackedBy[MAX_PLAYER_COUNT]; ///< For each player, have they attacked
                                       ///< me?
  ...
  typedef std::list<KindOfPercentProductionChange *>
      KindOfPercentProductionChangeList;
  typedef KindOfPercentProductionChangeList::iterator
  ...
  typedef SpecialPowerReadyTimerList::iterator
      SpecialPowerReadyTimerListIterator;
  ...
  // AI Assist flag - allows AI to help human player
